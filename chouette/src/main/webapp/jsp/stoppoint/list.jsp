...
<%-- Filtre --%>
<div>
  <s:form action="list" namespace="/stoppoint">
...
<%-- Actions --%>
<div class="actions">
  <s:url action="add"  namespace="/stoppoint" id="editPositionGeographique">
    <s:param name="typePositionGeographique" value="%{typePositionGeographique}" />
  </s:url>
...
<div id="displaytag"> 
  ...
  <display:table name="positionGeographiques" pagesize="20" requestURI="" id="positionGeographique" export="false">
    ...
    <display:column title="Action" sortable="false">
      <%-- BOUTON EDITER --%>
      <s:url id="editUrl" action="edit" namespace="/stoppoint">
      ...
      <s:a href="#" onclick="javascript:initialiserFusionArret(%{#attr.positionGeographique.id}, this);">
        <img border="0" src="<s:url value='/images/ajouter.png'/>" title="<s:text name="tooltip.fusionner"/>">
      </s:a>&nbsp;&nbsp;

      <%-- BOUTON SUPPRIMER --%>
      <s:url id="deletePositionGeographique" action="delete" namespace="/stoppoint">
        <s:param name="idPositionGeographique">${positionGeographique.id}</s:param>
