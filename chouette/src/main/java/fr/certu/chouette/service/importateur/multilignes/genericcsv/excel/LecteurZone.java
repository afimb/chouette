		...
		if (ligneCSV[6] != null)
			if (ligneCSV[6].trim().length() > 0) {
				...
				if (positionGeographique == null) {
					...
					if (!created) {
						logger.debug("CREATION DE ZONE COMMERCIALE : "+ligneCSV[6].trim());
						positionGeographique = new PositionGeographique();
						positionGeographique.setName(ligneCSV[6].trim());
						zones.put(positionGeographique.getName(), positionGeographique);
						counter++;
						...
						if (ligneCSV[4] != null)
							if (ligneCSV[4].trim().length() > 0)
								positionGeographique.setStreetName(ligneCSV[4].trim()); // Adresse
						if (ligneCSV[5] != null)
					...
					logger.debug("RECHARGEMENT DE ZONE COMMERCIALE : "+ligneCSV[6].trim());
				...
				if (ligneCSV[7] != null)
					if (ligneCSV[7].trim().length() > 0) {
						...
						if (arretPhysique == null) {
							...
							if (ligneCSV[5] != null)
								if (ligneCSV[5].trim().length() > 0)
									arretPhysique.setCountryCode(ligneCSV[5].trim());
