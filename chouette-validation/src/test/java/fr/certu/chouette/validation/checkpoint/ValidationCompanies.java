...
import fr.certu.chouette.validation.checkpoint.CompanyCheckPoints;
...
public class ValidationCompanies extends AbstractTestValidation
{
   ...
   private List<Company> beans = new ArrayList<>();
   
   @BeforeGroups (groups = { "company" })
   public void init()
   {
      checkPoint = (CompanyCheckPoints) applicationContext
            .getBean("companyCheckPoints");
      long id = 1;
      ...
      try
      {
         ...
         parameters.put("check_company","1");
   ...
   @Test(groups = { "company" }, description = "4-Company-1 no test")
   public void verifyTest4_1_notest() throws ChouetteException
   {
      ...
      parameters.put("check_company","0");
      ...
      Assert.assertEquals(report.getItems().get(0).getMessageKey(), "4-Company-1", " report must have 1 item on correct key");
      Assert.assertEquals(report.getItems().get(0).getItems().size(), 0, " checkpoint must have no detail");

   }
   
   @Test(groups = { "company" }, description = "4-Company-1 unicity")
   public void verifyTest4_1_unique() throws ChouetteException
   {
      ...
      // unique
      JSONObject column = parameters.getJSONObject("company").getJSONObject("objectid");
      column.put("unique",1);
      ...
      DetailReportItem detail = checkReportForTest4_1(report,"4-Company-1",bean2.getObjectId());
   ...
   @Test(groups = { "company" }, description = "4-Company-1 pattern numeric")
   public void verifyTest4_1_pattern_numeric() throws ChouetteException
   {
      ...
      JSONObject column = parameters.getJSONObject("company").getJSONObject("registration_number");
      ...
      checkReportForTest4_1(report,"4-Company-1");
   }

   
   @Test(groups = { "company" }, description = "4-Company-1 pattern alphabetic")
   ...
   @Test(groups = { "company" }, description = "4-Company-1 pattern uppercase")
   ...
   @Test(groups = { "company" }, description = "4-Company-1 pattern lowercase")
   ...
   @Test(groups = { "company" }, description = "4-Company-1 min_size alpha")
   ...
   @Test(groups = { "company" }, description = "4-Company-1 max_size alpha")
   ...
   @Test(groups = { "company" }, description = "4-Company-1 min_size numeric")
   ...
   @Test(groups = { "company" }, description = "4-Company-1 max_size numeric")
