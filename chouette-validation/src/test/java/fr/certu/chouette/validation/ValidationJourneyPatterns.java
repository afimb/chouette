...
public class ValidationJourneyPatterns extends AbstractValidation
{
	...
	public void verifyTest1() throws ChouetteException 
	{
		...
		JourneyPatternCheckPoints checkPoint = (JourneyPatternCheckPoints) applicationContext.getBean("journeyPatternCheckPoints");
		...
		List<Line> beans = LineLoader.load(importLine, "src/test/data/3-JourneyPattern-1.xml");
		...
		{
			route.setId(id++);
			if (route.getJourneyPatterns().size() == 2) route1 = route;
		...
		for (ReportItem item : report.getItems()) 
		{
			...
			if (checkPointReport.getMessageKey().equals("3-JourneyPattern-1"))
			{
				...
				for (ReportItem ditem : checkPointReport.getItems()) 
				{
					...
					if (detailReport.getObjectId().equals(jp1.getObjectId())) objectIdFound = true;
				}
				Assert.assertTrue(objectIdFound,"detail report must refer JourneyPattern 1");
			}
		}
		Assert.assertTrue(found,"report must contain a 3-JourneyPattern-1 checkPoint");
