...
public class ValidationStopAreas extends AbstractTransactionalTestNGSpringContextTests
{
	...
	public void verifyTest1() throws ChouetteException 
	{
		...
		INeptuneManager<StopArea> stopAreaManager = (INeptuneManager<StopArea>) applicationContext.getBean("stopAreaManager");
	...
	public void verifyTest3() throws ChouetteException 
	{
		...
		for (StopArea stopArea : beans) 
		{
			if (stopArea.getObjectId().equals("NINOXE:StopArea:15568801")) // St Paul
			{
				area1 = stopArea;
			}
			if (stopArea.getObjectId().equals("NINOXE:StopArea:15568802")) // place de verdun
			{
				area2 = stopArea;
			}
	...
	public void verifyTest4() throws ChouetteException 
	{
		...
		{
			area.complete();	
			if (area.getAreaType().equals(ChouetteAreaEnum.ITL)) continue;
			if (area.getLatitude().doubleValue() > maxLat) maxLat = area.getLatitude().doubleValue();
			if (area.getLatitude().doubleValue() < minLat) minLat = area.getLatitude().doubleValue();
			if (area.getLongitude().doubleValue() > maxLon) maxLon = area.getLongitude().doubleValue();
			if (area.getLongitude().doubleValue() < minLon) minLon = area.getLongitude().doubleValue();
		}
