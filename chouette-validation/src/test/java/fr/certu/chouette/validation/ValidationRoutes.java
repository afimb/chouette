...
public class ValidationRoutes extends AbstractValidation
{
   ...
   public void verifyTest1() throws ChouetteException
   {
      ...
      RouteCheckPoints checkPoint = (RouteCheckPoints) applicationContext
      ...
      route1.getStopPoints()
            .get(1)
            .setContainedInStopArea(
                  route1.getStopPoints().get(0).getContainedInStopArea());
      ...
      for (ReportItem item : report.getItems())
      {
         ...
         if (checkPointReport.getMessageKey().equals("3-Route-1"))
         {
            ...
            for (ReportItem ditem : checkPointReport.getItems())
            {
               ...
               if (detailReport.getObjectId().equals(route1.getObjectId()))
                  route1objectIdFound = true;
            }
            Assert.assertTrue(route1objectIdFound,
                  "detail report must refer route 1");
         }
      }
   ...
   public void verifyTest2() throws ChouetteException
   {
      ...
      StopArea area1 = route1.getStopPoints().get(1).getContainedInStopArea()
            .getParent();
      ...
      for (ReportItem item : report.getItems())
      {
         ...
         if (checkPointReport.getMessageKey().equals("3-Route-2"))
         {
            ...
            for (ReportItem ditem : checkPointReport.getItems())
            {
               ...
               if (detailReport.getObjectId().equals(route1.getObjectId()))
                  route1objectIdFound = true;
               if (detailReport.getObjectId().equals(route2.getObjectId()))
                  route2objectIdFound = true;
            }
            Assert.assertTrue(route1objectIdFound,
                  "detail report must refer route 1");
            Assert.assertTrue(route2objectIdFound,
                  "detail report must refer route 2");
         }
      }
   ...
   public void verifyTest3() throws ChouetteException
   {
      ...
      {
         StopArea area1 = route1.getStopPoints().get(i)
      ...
      {
         StopArea area1 = route2.getStopPoints().get(i)
               .getContainedInStopArea();
         double distance = distance(area0, area1);
         if (distance > distanceMax)
            distanceMax = distance;
      ...
      parameters.getJSONObject("mode_bus").put("inter_stop_area_distance_min",
            (int) distanceMin + 10);
      parameters.getJSONObject("mode_bus").put("inter_stop_area_distance_max",
            (int) distanceMax - 10);
      PhaseReportItem report = new PhaseReportItem(PHASE.THREE);
      ...
      for (ReportItem item : report.getItems())
      {
         ...
         if (checkPointReport.getMessageKey().equals("3-Route-3"))
         {
            ...
            Assert.assertEquals(checkPointReport.getItems().size(), 4,
                  " checkPointReport must have 4 item");
            ...
            for (ReportItem ditem : checkPointReport.getItems())
            {
               DetailReportItem detailReport = (DetailReportItem) ditem;
               if (detailReport.getObjectId().equals(route1.getObjectId()))
                  routeobjectIdFound = true;
               if (detailReport.getObjectId().equals(route2.getObjectId()))
                  routeobjectIdFound = true;
            }
            Assert.assertTrue(routeobjectIdFound,
                  "detail report must refer route 1 or 2 ");
         }
      }
   ...
   public void verifyTest4() throws ChouetteException
   {
      ...
      Assert.assertEquals(report.getStatus(), Report.STATE.ERROR,
            " report must be on level error"); // cause test 7 no JP
      Assert.assertEquals(report.hasItems(), true, " report must have items");
      boolean found = false;
      for (ReportItem item : report.getItems())
      {
   ...
   public void verifyTest8() throws ChouetteException
   {
      ...
      route1.getJourneyPatterns()
            .get(0)
            .removeStopPoint(
                  route1.getJourneyPatterns().get(0).getStopPoints().get(0));
      route1.setObjectId("NINOXE:Route:first");
