...
>>>>>>> 4df7cfa0653837bac9b8741b14a4d57a5e15cd07
        ...
        if (vehicleJourneys != null) {
            for (int i = 0; i < vehicleJourneys.size(); i++) {
                ...
                if (values != null && !values.isEmpty()) {
                    ...
                    Set<VehicleJourney> vehicleJourneySet = new HashSet<VehicleJourney>();
                    vehicleJourneySet.add(vehicleJourney);
                    map.put(vehicleJourney.getRouteId(), vehicleJourneySet);
                }
                if (vehicleJourney.getRoute() != null) {
                    if (!vehicleJourney.getRouteId().equals(vehicleJourney.getRoute().getObjectId())) {
                        ReportItem detailReportItem = new DetailReportItem("Test2_Sheet17_Step1_error", Report.STATE.ERROR, vehicleJourney.getObjectId(), vehicleJourney.getRouteId());
...
=======
            ...
            for (String key : keys) {
                Set<VehicleJourney> vehicleJourneySet = map.get(key);
                for (VehicleJourney vehicleJourney : vehicleJourneySet) {
/*                    
                }
            }
