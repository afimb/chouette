...
public class ConnectionLinkCheckPoints extends AbstractValidation implements ICheckPointPlugin<ConnectionLink>
{
			...
			PhaseReportItem report) 
	{
		...
		initCheckPoint(report, CONNECTION_LINK_1, CheckPointReportItem.SEVERITY.WARNING);
		initCheckPoint(report, CONNECTION_LINK_2, CheckPointReportItem.SEVERITY.WARNING);
		initCheckPoint(report, CONNECTION_LINK_3, CheckPointReportItem.SEVERITY.WARNING);
	...
	private void checkConnectionLink1_2(PhaseReportItem report,
		...
		if (distance > distanceMax)
		{
			...
			DetailReportItem detail = new DetailReportItem(CONNECTION_LINK_1,connectionLink.getObjectId(), Report.STATE.WARNING, location,map);
			...
			ConnectionLink connectionLink, JSONObject parameters) 
	{
		...
		checkLinkSpeed(report, connectionLink, connectionLink.getFrequentTravellerDuration(), distance, maxFrequentSpeed, CONNECTION_LINK_3,"_3");
		checkLinkSpeed(report, connectionLink, connectionLink.getMobilityRestrictedTravellerDuration(), distance, maxMobilitySpeed, CONNECTION_LINK_3,"_4");
