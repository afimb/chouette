...
public class AccessLinkCheckPoints extends AbstractValidation implements ICheckPointPlugin<AccessLink>
{
			...
			PhaseReportItem report) 
	{
		...
		// 3-AccessLink-2 : check distance of link against distance between ends of AccessLink
		// 3-AccessLink-3 : check speeds in AccessLink
		initCheckPoint(report, ACCESS_LINK_1, CheckPointReportItem.SEVERITY.WARNING);
		initCheckPoint(report, ACCESS_LINK_2, CheckPointReportItem.SEVERITY.WARNING);
		initCheckPoint(report, ACCESS_LINK_3, CheckPointReportItem.SEVERITY.WARNING);
	...
	private void checkAccessLink1_2(PhaseReportItem report,
		...
		if (distance > distanceMax)
		{
			...
			DetailReportItem detail = new DetailReportItem(ACCESS_LINK_1,accessLink.getObjectId(), Report.STATE.WARNING, location,map);
	...
	private void checkAccessLink3(PhaseReportItem report,
			AccessLink accessLink, JSONObject parameters) 
	{
		// 3-AccessLink-3 : check speeds in accessLink
		...
		checkLinkSpeed(report, accessLink, accessLink.getFrequentTravellerDuration(), distance, maxFrequentSpeed, ACCESS_LINK_3,"_3");
		checkLinkSpeed(report, accessLink, accessLink.getMobilityRestrictedTravellerDuration(), distance, maxMobilitySpeed, ACCESS_LINK_3,"_4");
