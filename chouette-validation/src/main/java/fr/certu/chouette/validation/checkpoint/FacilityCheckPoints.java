...
public class FacilityCheckPoints extends AbstractValidation implements ICheckPointPlugin<Facility>
{
	...
	private void checkFacility1(PhaseReportItem report, Facility facility) 
	{
		...
		if (!hasCoordinates(facility)) 
		{
			...
			DetailReportItem detail = new DetailReportItem(FACILITY_1, facility.getObjectId(), Report.STATE.ERROR, location, map);
			addValidationError(report, FACILITY_1, detail);
		}
	}

	private void checkFacility2(PhaseReportItem report, 
			 Facility facility, JSONObject parameters) {
		// 3-Facility-2 : check distance of facility with stop area ()
		if (!hasCoordinates(facility)) return; 
		if (facility.getContainedInStopArea() == null) return;
		StopArea area = facility.getContainedInStopArea();
		if (!hasCoordinates(area)) return; 
		long distanceMax = parameters.optLong(FACILITY_STOP_AREA_DISTANCE_MAX,300);
			...
			if (distance > distanceMax)
			{
				...
				DetailReportItem detail = new DetailReportItem(FACILITY_2,facility.getObjectId(), Report.STATE.WARNING, location,map);
