...
public class AccessPointConverter extends GenericConverter
{
   private static final Logger logger = Logger
         .getLogger(AccessPointConverter.class);
   ...
   public AccessPointConverter(VTDNav vTDNav) throws XPathParseException,
   ...
   public List<AccessPoint> convert() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      ...
      while (pilot.evalXPath() != -1)
      {
         ...
         // Optionnal
         accessPoint
               .setLongitude(new BigDecimal((String) parseOptionnalSubElement(
                     nav, "Location", "Longitude")));
         accessPoint.setLatitude(new BigDecimal(
               (String) parseOptionnalSubElement(nav, "Location", "Latitude")));
         // Force type to WGS84
         ...
         accessPoint.setOpeningTime((Time) parseOptionnalSubElement(nav,
               "TimeBand", "StartTime", "Time"));
         accessPoint.setClosingTime((Time) parseOptionnalSubElement(nav,
               "TimeBand", "EndTime", "Time"));
         accessPoint.setComment((String) parseOptionnalElement(nav,
               "Description"));
         ...
         {
            List<AccessPoint> aps = accessPointsByStopPlaceObjectId
