...
public class AccessLinkConverter extends GenericConverter
{
   private static final Logger logger = Logger
         .getLogger(ConnectionLinkConverter.class);
   ...
   public AccessLinkConverter(VTDNav vTDNav) throws XPathParseException,
   ...
   public List<AccessLink> convert() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      ...
      while (pilot.evalXPath() != -1)
      {
         ...
         Object objectVersion = parseOptionnalAttribute(nav, "version",
               "Integer");
         link.setObjectVersion(objectVersion != null ? (Integer) objectVersion
               : 0);

         link.setName((String) parseOptionnalElement(nav, "Name"));
         link.setComment((String) parseOptionnalElement(nav, "Description"));
         link.setLinkDistance((BigDecimal) parseOptionnalElement(nav,
               "Distance", "BigDecimal"));
         link.setLinkType((ConnectionLinkTypeEnum) parseOptionnalElement(nav,
               "Covered", "ConnectionLinkTypeEnum"));
         link.setDefaultDuration((Time) parseOptionnalElement(nav,
               "DefaultDuration", "Duration"));
         link.setFrequentTravellerDuration((Time) parseOptionnalElement(nav,
               "FrequentTravellerDuration", "Duration"));
         link.setOccasionalTravellerDuration((Time) parseOptionnalElement(nav,
               "OccasionalTravellerDuration", "Duration"));
         link.setMobilityRestrictedTravellerDuration((Time) parseOptionnalElement(
               nav, "MobilityRestrictedTravellerDuration", "Duration"));
         ...
         {
            List<AccessLink> alks = accessLinksByStopPlaceObjectId
                  .get(stopAreaObjectId);
            alks.add(link);
   ...
   private String subXpathSelection(String xPath) throws XPathParseException
   {
      ...
      localAutoPilot.declareXPathNameSpace("gml",
