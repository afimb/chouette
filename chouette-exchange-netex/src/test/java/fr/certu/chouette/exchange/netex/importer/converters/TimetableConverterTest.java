...
public class TimetableConverterTest extends AbstractTestNGSpringContextTests
{
   ...
   private Timetable getByObjectId(String objectId) throws XPathEvalException,
         NavException, XPathParseException, ParseException
   {
      ...
      Assert.assertNotNull(selectedTimetable,
            "can't find expected timetable having " + objectId + " as objectId");
   ...
   public void verifyShortName() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Timetable selectedTimetable = getByObjectId("T:DayType:1");
   ...
   public void verifyPeriod() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Timetable selectedTimetable = getByObjectId("T:DayType:1");
      Assert.assertEquals(selectedTimetable.getPeriods().size(), 5);
      Assert.assertEquals(selectedTimetable.getPeriods().get(0).getStartDate(),
            dateFormat.parse("2040-03-18"));
      Assert.assertEquals(selectedTimetable.getPeriods().get(0).getEndDate(),
            dateFormat.parse("2040-04-22"));
   }

   @Test(groups = { "ServiceCalendarFrame" }, description = "DayType's CalendarDays attribute reading")
   public void verifyCalendarDay() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Timetable selectedTimetable = getByObjectId("T:DayType:1");
      Assert.assertEquals(selectedTimetable.getCalendarDays().size(), 5);
      Assert.assertEquals(selectedTimetable.getCalendarDays().get(0).getDate(),
            dateFormat.parse("2015-03-18"));
   }

   @Test(groups = { "ServiceCalendarFrame" }, description = "DayType's properties attribute reading")
   public void verifyProperties() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Timetable selectedTimetable = getByObjectId("T:DayType:1");
