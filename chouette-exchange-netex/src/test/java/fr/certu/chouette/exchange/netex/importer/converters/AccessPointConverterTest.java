...
public class AccessPointConverterTest extends AbstractTestNGSpringContextTests
{
   ...
   {
      File f = FileUtils.getFile("src", "test", "resources",
            "line_with_connections.xml");
      ;
   ...
   public void verifyCoordinates() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      List<AccessPoint> accessPoints = accessPointConverter.convert();
      Assert.assertEquals(accessPoints.get(0).getLongitude(), new BigDecimal(
            "2.37300000000000022026824808563105762004852294921875"));
      Assert.assertEquals(accessPoints.get(0).getLatitude(), new BigDecimal(
            "48.7999999999999971578290569595992565155029296875"));
   }

   private AccessPoint getByObjectId(String objectId)
         ...
         ParseException
   {
      ...
      Assert.assertNotNull(selectedAccess,
            "can't find expected accessPoint having " + objectId
                  + " as objectId");
      return selectedAccess;
   }

   @Test(groups = { "SiteFrame" }, description = "AccessPoint's containedInStopArea attribute reading")
   public void verifyContainedInStopArea() throws XPathEvalException,
         NavException, XPathParseException, ParseException
   {
      ...
      Assert.assertEquals(selectedAccess.getContainedInStopArea(),
            "T:StopArea:PLC-0");
   }
