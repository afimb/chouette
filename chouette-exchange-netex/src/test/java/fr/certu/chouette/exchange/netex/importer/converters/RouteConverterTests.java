...
public class RouteConverterTests extends AbstractTestNGSpringContextTests
{
      ...
      for (Route route : routes)
      {
         if (route.getObjectId().equals("T:Route:1-1"))
         {
            ...
            Assert.assertEquals(route.getStopPoints().get(0).getObjectId(),
                  "T:StopPoint:1-1-0");
            Assert.assertEquals(route.getStopPoints().get(0)
                  .getContainedInStopAreaId(), "T:Quay:0");
         }
      }

   }

   private Route getRouteByObjectId(String objectId) throws XPathEvalException,
         NavException, XPathParseException, ParseException
   {
      ...
      Assert.assertNotNull(selectedRoute, "can't find expected route having "
            + objectId + " as objectId");
      return selectedRoute;
   }

   @Test(groups = { "ServiceFrame" }, description = "Route's name attribute reading")
   public void verifyRouteName() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
   ...
   public void verifyWayBackRouteId() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Route selectedRoute = getRouteByObjectId("T:Route:1-1");
   ...
   public void verifyRoutePublishedName() throws XPathEvalException,
         NavException, XPathParseException, ParseException
   {
      Route selectedRoute = getRouteByObjectId("T:Route:1-1");
      Assert.assertEquals(selectedRoute.getPublishedName(),
            "Gare vers Jean Monnet");
   }

   @Test(groups = { "ServiceFrame" }, description = "Route's direction attribute reading")
   public void verifyRouteDirection() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Route selectedRoute = getRouteByObjectId("T:Route:1-1");
   ...
   public void verifyRouteWayback() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Route selectedRoute = getRouteByObjectId("T:Route:1-1");
   ...
   public void verifyRouteComment() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      Route selectedRoute = getRouteByObjectId("T:Route:1-1");
   ...
   public void readStopPointObjectIdFromPointOnRouteId()
   {
      ...
      Assert.assertEquals(
            routeConverter.readStopPointObjectIdFromPointOnRouteId(exemple),
            "1-2-3");
   }
