...
public class ServiceCalendarFrameFileWritterTests extends
      AbstractTestNGSpringContextTests
{
   ...
   private Line prepareModel() throws CreateModelException, PolicyException
   {
      ...
      VehicleJourney vehicleJourney = modelFactory
            .createModel(VehicleJourney.class);
   ...
   private List<Timetable> createTimetables(int counter)
   ...
   {
      Assert.assertEquals(
            xPath.evaluate("count(//netex:ServiceCalendarFrame)", xmlDocument),
            "2", "Must have 2 ServiceCalendarFrame");
   }
   ...
   public void verifyDayTypes() throws XPathExpressionException
   {
      for (Timetable timetable : timetables)
      {
         Assert.assertNotNull(xPath.evaluate(
               "//netex:ServiceCalendarFrame//netex:DayType[@id='"
                     + modelTranslator.netexId(timetable) + "']", xmlDocument,
               XPathConstants.NODE), "Must find DayType");
         ...
         for (DayTypeEnum dayType : timetable.getDayTypes())
         {
            Assert.assertNotNull(xPath.evaluate(
                  "//netex:ServiceCalendarFrame//netex:DayType[@id='"
                        ...
                        + "']//netex:PropertyOfDay[netex:DaysOfWeek = '"
                        + dayType.name() + "']", xmlDocument,
                  XPathConstants.STRING), "daysOfWeek must exists");
         }
      }
   }
