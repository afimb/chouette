...
public class ComplexModelFactoryTest extends AbstractTestNGSpringContextTests
{
   ...
   public void verifyRoute()
   {
      ...
      Assert.assertEquals(route.getJourneyPatterns().size(),
            complexModelFactory.getJourneyPatternCount());
      Assert.assertEquals(route.getStopPoints().size(),
            complexModelFactory.getRouteStopCount());
   }
   ...
   public void verifyGroupOfLine()
   {
      ...
      Assert.assertEquals(complexModelFactory.nominalLine("1")
            .getGroupOfLines().size(),
   ...
   public void verifyVehicleJourneysOnJourneyPattern()
   {
      ...
      for (int i = 0; i < journeyPatternCount; i++)
      {
         ...
         {
            Assert.assertEquals(vj.getJourneyPattern().getObjectId(),
                  jp.getObjectId());
         }
      }
   }
