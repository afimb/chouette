...
public class AccessLinkTest extends ChouetteModelTest
{
   ...
   public void verifyAccessLink() throws XPathExpressionException,
         ParseException
   {
   ...
   public void verifyaccessLinkName() throws XPathExpressionException,
         ParseException
   {
      for (AccessLink accessLink : accessLinks)
      {
         assertXPathTrue("boolean(//netex:PathLink[@id = '"
               + modelTranslator.netexId(accessLink) + "']/netex:Name/text()='"
               + accessLink.getName() + "')");
      }
   }

   @Test(groups = { "ServiceFrame", "accessLinks" }, description = "Validate presence of StopPlaceEntranceRef")
   public void verifyaccessLinkStartAndEnd() throws XPathExpressionException,
         ParseException
   {
      for (AccessLink accessLink : accessLinks)
      {
         ...
         if (accessLink.getLinkOrientation().equals(
         ...
         String xPathExpr = "boolean(//netex:PathLink" + "[@id = '"
               + modelTranslator.netexId(accessLink) + "']"
               + "/netex:To/netex:PlaceRef/@ref='"
               + modelTranslator.netexId(startLink) + "')";
         assertXPathTrue("boolean(//netex:PathLink" + "[@id = '"
               + modelTranslator.netexId(accessLink) + "']"
               + "/netex:To/netex:PlaceRef/@ref='"
               + modelTranslator.netexId(endLink) + "')");

         assertXPathTrue("boolean(//netex:PathLink" + "[@id = '"
               + modelTranslator.netexId(accessLink) + "']"
               + "/netex:From/netex:EntranceRef/@ref='"
               + modelTranslator.netexId(startLink) + "')");
      }
   ...
   public void verifyDefaultDuration() throws XPathExpressionException,
         ParseException
   {
      for (AccessLink accessLink : accessLinks)
      {
         assertXPathTrue("boolean(//netex:PathLink[@id = '"
               + getId(accessLink)
               + "']/netex:TransferDuration/netex:DefaultDuration/text()='"
               + durationFactory.newDuration(accessLink.getDefaultDuration()
                     .getTime()) + "')");
      }
   }
   ...
   public void verifyFrequentTravellerDuration()
         throws XPathExpressionException, ParseException
   {
      for (AccessLink accessLink : accessLinks)
      {
         assertXPathTrue("boolean(//netex:PathLink[@id = '"
               + getId(accessLink)
               + "']/netex:TransferDuration/netex:DefaultDuration/text()='"
               + durationFactory.newDuration(accessLink
