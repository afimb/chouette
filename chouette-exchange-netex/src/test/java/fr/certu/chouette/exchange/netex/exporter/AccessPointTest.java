...
public class AccessPointTest extends ChouetteModelTest
{
   ...
   public void verifyAccessPoint() throws XPathExpressionException,
         ParseException
   {
      assertXPathCount("count(//netex:entrances/netex:StopPlaceEntrance)", 1);

      for (AccessPoint accessPoint : accessPoints())
      {
         assertXPathCount(
               "count(//netex:entrances/netex:StopPlaceEntrance[@id = '"
   ...
   public void verifyAccessPointName() throws XPathExpressionException,
         ParseException
   {
      for (AccessPoint accessPoint : accessPoints())
      {
   ...
   public void verifyAccessPointCoordinates() throws XPathExpressionException,
         ParseException
   {
      for (AccessPoint accessPoint : accessPoints())
      {
         ...
         assertXPathTrue("boolean(//netex:StopPlaceEntrance[@id = '"
               + modelTranslator.netexId(accessPoint)
               + "']/netex:Centroid/netex:Location/netex:Longitude/text()='"
               + accessPoint.getLongitude().toPlainString() + "')");

         assertXPathTrue("boolean(//netex:StopPlaceEntrance[@id = '"
               + modelTranslator.netexId(accessPoint)
               + "']/netex:Centroid/netex:Location/netex:Latitude/text()='"
               + accessPoint.getLatitude().toPlainString() + "')");
      }
