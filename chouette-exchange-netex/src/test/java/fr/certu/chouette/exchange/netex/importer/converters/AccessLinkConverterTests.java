...
public class AccessLinkConverterTests
{
   private static final Logger logger = Logger
         .getLogger(ConnectionLinkConverterTests.class);
   ...
   public void verifySequenceLength() throws XPathEvalException, NavException,
   ...
   private AccessLink getConnectionLinkByObjectId(String objectId)
         ...
         ParseException
   {
      ...
      Assert.assertNotNull(selectedLink,
            "can't find expected SiteConnection having " + objectId
                  + " as objectId");
      return selectedLink;
   }

   @Test(groups = { "ServiceFrame" }, description = "ConnectionLink's DefaultDuration attribute reading")
   public void verifyDefaultDuration() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getDefaultDuration(), new Time(
            3 * 60 * 1000));
   }

   @Test(groups = { "ServiceFrame" }, description = "ConnectionLink's FrequentTravellerDuration attribute reading")
   public void verifyFrequentTravellerDuration() throws XPathEvalException,
         NavException, XPathParseException, ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getFrequentTravellerDuration(),
            new Time(2 * 60 * 1000));
   }

   @Test(groups = { "ServiceFrame" }, description = "ConnectionLink's OccasionalTravellerDuration attribute reading")
   public void verifyOccasionalTravellerDuration() throws XPathEvalException,
         NavException, XPathParseException, ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getOccasionalTravellerDuration(),
            new Time(4 * 60 * 1000));
   }

   @Test(groups = { "ServiceFrame" }, description = "ConnectionLink's MobilityRestrictedTravellerDuration attribute reading")
   public void verifyMobilityRestrictedTravellerDuration()
         throws XPathEvalException, NavException, XPathParseException,
         ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(
            selectedLink.getMobilityRestrictedTravellerDuration(), new Time(
                  8 * 60 * 1000));
   }

   @Test(groups = { "ServiceFrame" }, description = "ConnectionLink's linkDistance attribute reading")
   public void verifyLinkDistance() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getLinkDistance(), new BigDecimal(
            "155.00"));
   }

   @Test(groups = { "ServiceFrame" }, description = "ConnectionLink's linkType attribute reading")
   public void verifyLinkType() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getLinkType(),
            ConnectionLinkTypeEnum.Underground);
   }
   ...
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getName(),
            "acces par la rue de la Chine");
   }
   ...
   public void verifyPlaceRef() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getEndOfLinkId(),
            "RATP_PIVI:Quay:5246066");
   }
   ...
   public void verifyEntranceRef() throws XPathEvalException, NavException,
         XPathParseException, ParseException
   {
      AccessLink selectedLink = getConnectionLinkByObjectId("RATP_PIVI:PathLink:1234");
      Assert.assertEquals(selectedLink.getStartOfLinkId(),
            "RATP_PIVI:Place:2345");
   }
}
