...
@Log4j
public class TimeTableFrameFileWritterTests extends
      AbstractTestNGSpringContextTests
{
   ...
   {
      Assert.assertEquals(
            xPath.evaluate("count(//netex:ServiceJourney)", xmlDocument), "28");
      ...
      Assert.assertNotNull(xPath.evaluate("//netex:ServiceJourney[@id='"
            + modelTranslator.netexId(vehicle) + "']", xmlDocument,
            XPathConstants.NODE));
   }
   ...
   public void verifyDayTypes() throws XPathExpressionException
   {
      Assert.assertEquals(xPath.evaluate(
            "count(//netex:ServiceJourney//netex:DayTypeRef)", xmlDocument),
            "56");
      ...
      for (int i = 0; i < vehicles.get(0).getTimetables().size(); i++)
      {
         ...
         Assert.assertNotNull(xPath.evaluate(
               "//netex:ServiceJourney//netex:DayTypeRef[@ref='"
