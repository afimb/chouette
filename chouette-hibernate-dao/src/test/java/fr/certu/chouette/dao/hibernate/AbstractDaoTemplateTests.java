      ...
      extends AbstractTransactionalTestNGSpringContextTests
{
   private static final Logger logger = Logger
         .getLogger(AbstractDaoTemplateTests.class);
   ...
   {
      daoTemplate = (HibernateDaoTemplate<T>) applicationContext
            .getBean(daoName);
   ...
   public void verifyGet()
   {
      ...
      Assert.assertFalse(newBean.getId().equals(Long.valueOf(0)),
            "found Bean should have id different of zero");
      Assert.assertTrue(newBean.getId().equals(id),
            "found Bean should have asked id=" + id + "");
   }
   ...
   public void verifyGetAll()
   {
      ...
      Assert.assertTrue(beans.size() > 0,
            "size of returned list shouldn't be zero");
      System.err.println(beans.get(0).toString(" ", 0));
   }
   ...
   public void verifySelect()
   {
      ...
      Assert.assertTrue(beans.size() <= 2,
            "size of returned list(" + beans.size() + ") should be less than 2");
   }
   ...
   public void verifySelectFilter()
   {
      refreshBean();
      // TODO : see how to put prepared objects in test database
      List<FilterData> list = getSelectFilters();
      for (FilterData filterData : list)
      {
         Filter filter = filterData.getFilter();
         if (filterData.getResultCount() < 0)
            filter.addLimit(2);
         List<T> beans = daoTemplate.select(filter);
         // for (T t : beans)
         // {
         ...
         if (filterData.getResultCount() >= 0)
         {
            Assert.assertEquals(beans.size(), filterData.getResultCount(),
                  "Filter " + filterData.getName() + ": size of returned list("
                        + beans.size() + ") invalid");
         } else
         {
            Assert.assertTrue(beans.size() <= 2,
                  "Filter " + filterData.getName() + ": size of returned list("
                        + beans.size() + ") should be less than 2");
         }
      }

   }

   // @Test (groups = {"hibernate","hql"}, description =
   // "daoTemplate should return maximum 2 occurences of bean with a filter" )
   ...
   // }
   // Assert.assertTrue(beans.size() >
   // 0,"size of returned list("+beans.size()+") should be greater than 0");
   //
   // }
   ...
   protected List<FilterData> getSelectFilters()
   {
      ...
      ret.add(new FilterData("empty", Filter.getNewEmptyFilter(), -1));
      return ret;
   }
   ...
   {
      // HibernateDaoTemplate<TimeSlot> timeSlotTemplate =
      // (HibernateDaoTemplate<TimeSlot>)
      // applicationContext.getBean("timeSlotDao");
      ...
      HibernateDaoTemplate<Route> routeTemplate = (HibernateDaoTemplate<Route>) applicationContext
            .getBean("routeDao");
      HibernateDaoTemplate<VehicleJourney> vjTemplate = (HibernateDaoTemplate<VehicleJourney>) applicationContext
            .getBean("vehicleJourneyDao");
      ...
      vjTemplate.save(vehicleJourney);
   ...
   protected Timetable createTimetable()
   {
      ...
      timetable
            .addPeriod(new Period(toDate("01/11/2011"), toDate("01/11/2012")));

      HibernateDaoTemplate<Timetable> template = (HibernateDaoTemplate<Timetable>) applicationContext
            .getBean("timetableDao");
      template.save(timetable);
