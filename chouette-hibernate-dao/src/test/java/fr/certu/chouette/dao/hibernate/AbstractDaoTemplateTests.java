...
//			}
			if (filterData.getResultCount() >= 0)
			{
				Assert.assertEquals(beans.size(), filterData.getResultCount(),"Filter "+filterData.getName()+": size of returned list("+beans.size()+") invalid");
			}
			else
			{
				Assert.assertTrue(beans.size() <= 2,"Filter "+filterData.getName()+": size of returned list("+beans.size()+") should be less than 2");
			}
		}
	...
	protected VehicleJourney createVehicleJourney() 
	{
		...
		HibernateDaoTemplate<Line> lineTemplate = (HibernateDaoTemplate<Line>) applicationContext.getBean("lineDao");
		HibernateDaoTemplate<Route> routeTemplate = (HibernateDaoTemplate<Route>) applicationContext.getBean("routeDao");
		HibernateDaoTemplate<VehicleJourney> vjTemplate = (HibernateDaoTemplate<VehicleJourney>) applicationContext.getBean("vehicleJourneyDao");
