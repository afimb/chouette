...
public class ChouetteSessionFactoryBean extends LocalSessionFactoryBean 
{
	...
	@Setter private String strategy = "validate" ;

	@Setter private String getForeignKeysSql ;
	@Setter private String dropConstraintKeySql ;
	@Setter private String addForeignKeySql ;

	@Setter private List<String> cascadeDeleteFkeys = new ArrayList<String>();
	@Setter private List<String> setNullDeleteFkeys = new ArrayList<String>();
	@Setter private List<String> noActionDeleteFkeys = new ArrayList<String>();

	@Setter private String getPrimaryKeysSql ;
	@Setter private String addPrimaryKeySql ;

	@Setter private List<PrimaryKeyDef> primaryKeys;
	...
	private class CheckForeignKeysWork implements Work
	{
		...
		public void execute(Connection connection) throws SQLException 
		{
			...
			for (String keyName : cascadeDeleteFkeys) 
			{
				FkeyConstraint fkey = foreignKeys.remove(keyName);
				if (fkey == null)
				{
					logger.error("Foreign key "+keyName+" missing in schema");
					throw new HibernateDaoRuntimeException(HibernateDaoExceptionCode.DATABASE_SCHEMA_MISSING_FOREIGN_KEY, keyName);
				}
				checkRule(stmt, fkey, "CASCADE");
			}
			for (String keyName : setNullDeleteFkeys) 
			{
				FkeyConstraint fkey = foreignKeys.remove(keyName);
				if (fkey == null)
				{
			...
			for (String keyName : noActionDeleteFkeys) 
			{
				FkeyConstraint fkey = foreignKeys.remove(keyName);
				if (fkey == null)
				{
		 ...
		 * @throws SQLException
		 */
		private void checkRule(Statement stmt, FkeyConstraint fkey, String rule)
		throws SQLException 
		{
			if (!fkey.getDeleteRule().equals(rule))
			{
				...
				{
					throw new HibernateDaoRuntimeException(HibernateDaoExceptionCode.DATABASE_SCHEMA_WRONG_FOREIGN_KEY_TYPE, fkey.getConstraintName(),rule);
				}	
				...
				stmt.executeUpdate(sql);
				String message = MessageFormat.format("ForeignKey {1} on table {0},column {2}: ON DELETE rule changed to {7}", args);
		...
		{
			@Getter private String constraintName;
			@Getter private String tableName;
			@Getter private String columnName;
			@Getter private String targetTable;
			@Getter private String targetColumn;
			@Getter private String matchOption;
			@Getter private String updateRule;
			@Getter @Setter private String deleteRule;
			...
			public FkeyConstraint(ResultSet rst) throws SQLException
			{
				...
				if (matchOption.equals("NONE")) matchOption="SIMPLE";
	...
	private class CheckPrimaryKeysWork implements Work
	{
		...
		public void execute(Connection connection) throws SQLException 
		{
			...
			for (PrimaryKeyDef keyDef : primaryKeys) 
			{
				...
				if (constraint == null)
				{
					if (strategy.equals("validate"))
					{
						throw new HibernateDaoRuntimeException(HibernateDaoExceptionCode.DATABASE_SCHEMA_MISSING_PRIMARY_KEY, keyDef.getKey(),keyDef.getTable());
					}
				}
				else
				{
							...
							constraint.getColumnNames().equals(keyDef.getColumns()))
					{
						if (strategy.equals("validate"))
						{
							throw new HibernateDaoRuntimeException(HibernateDaoExceptionCode.DATABASE_SCHEMA_WRONG_PRIMARY_KEY, constraint.getConstraintName(),keyDef.getTable());
						}
						String sql = MessageFormat.format(dropConstraintKeySql, constraint.toArray());
						stmt.executeUpdate(sql);
						logger.debug("removing old primary key definition "+constraint.getConstraintName()+" on "+constraint.getTableName());
					}
					else
					{
						continue;
		...
		private class PkeyConstraint
		{
			@Getter private String constraintName;
			@Getter private String tableName;
			@Getter private List<String> columnNames = new ArrayList<String>();
