...
public class FilterToHibernateClauseTranslator<T extends NeptuneObject>
{
   ...
   public Predicate translate(Filter clause, CriteriaBuilder builder,
         Root<T> root)
   {
      if (clause == null)
      {
   ...
   private Predicate translateCombined(Filter clause, CriteriaBuilder builder,
         Root<T> root)
   {
...

      {
         ...
         Predicate criterion = builder.and(
               translate(filters[0], builder, root),
               translate(filters[1], builder, root));
         for (int i = 2; i < filters.length; i++)
         {
            criterion = builder.and(criterion,
                  translate(filters[i], builder, root));
         }
         return criterion;
      }
      case OR:
   ...
   private Predicate translateTerminal(Filter clause, CriteriaBuilder builder,
         Root<T> root)
   {
      ...
      case LESS_OR_EQUALS:
         return builder.lessThanOrEqualTo(field,
               (Comparable) clause.getFirstValue());
      case GREATER:
         return builder.greaterThan(field, (Comparable) clause.getFirstValue());
      case GREATER_OR_EQUALS:
         return builder.greaterThanOrEqualTo(field,
               (Comparable) clause.getFirstValue());
      case LIKE:
         Expression<String> like = path.get(name);
         return builder.like(like,
               getILikeOrLikeRestrictionValue(clause.getFirstValue()));
      case ILIKE:
         Expression<String> ilike = path.get(name);
         return builder.like(builder.lower(ilike),
               getILikeOrLikeRestrictionValue(((String) clause.getFirstValue())
                     .toLowerCase()));
      case IN:
         ...
         return null;
      case BETWEEN:
         return builder.between(field, (Comparable) clause.getFirstValue(),
               (Comparable) clause.getSecondValue());
      case SQL_WHERE:
