...
public class ExportCommand
{
         ...
         Map<String, List<String>> parameters)
   {
      ...
      } catch (Exception e)
      {
         ...
         GuiReport errorReport = new GuiReport("EXPORT_ERROR",
               Report.STATE.ERROR);
         GuiReportItem item = new GuiReportItem(GuiReportItem.KEY.EXCEPTION,
               Report.STATE.ERROR, e.getMessage());
   ...
   private int saveExportReport(GuiExport export, String format, Report report,
         int position)
   {
      ...
      ExportLogMessage message = new ExportLogMessage(export, format, report,
            position++);
      ...
      if (report.getItems() != null)
      {
         ...
         {
            position = saveExportReportItem(export, format, item, prefix,
                  position);
         }
      }
      return position;
   ...
   private int saveExportReportItem(GuiExport export, String format,
         ReportItem item, String prefix, int position)
   {
      ExportLogMessage message = new ExportLogMessage(export, format, item,
            prefix, position++);
      exportLogMessageDao.save(message);
      if (item.getItems() != null)
      {
         ...
         {
            position = saveExportReportItem(export, format, child, subPrefix,
                  position);
         }
      }
      return position;
   ...
   private int saveExportReports(GuiExport export, String format,
         List<Report> reports)
   {
      int position = 1;
      int retVal = 1;
      ...
      for (Report report : reports)
      {
         ...
         if (report.getStatus().ordinal() <= Report.STATE.WARNING.ordinal())
            retVal = 0;
      }
      return retVal;
