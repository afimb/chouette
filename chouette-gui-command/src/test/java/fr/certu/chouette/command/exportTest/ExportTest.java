...
public class ExportTest extends AbstractTestNGSpringContextTests
{
   ...
   {
      IDaoTemplate<GuiExport> exportDao = (IDaoTemplate<GuiExport>) applicationContext
            .getBean("exportDao");
   ...
   public void verifyExportAll()
   {
      ...
      String[] args = { "-c", "export", "-exportid",
            Long.toString(exportTask.getId()), "-format", "NEPTUNE", "-o",
            "line", "-outputFile", f.getAbsolutePath() };
      ...
      // Assert.assertEquals(exportTask.getStatus(), "processing",
      // "exportTask should have status as processing");
      Assert.assertNotNull(exportTask.getResults(),
            "exportTask should have a result list");
      Assert.assertFalse(exportTask.getResults().isEmpty(),
            "exportTask should have a non empty result");
   ...
   public void verifyExportStopArea()
   {
      ...
      Command command = (Command) applicationContext.getBean("Command");
      String[] args = { "-c", "export", "-exportid",
            Long.toString(exportTask.getId()), "-format", "GTFS", "-o",
            "stoparea", "-outputFile", f.getAbsolutePath() };
      int code = command.execute(args);
      Command.closeDao();
      // check results
      Command.initDao();
