...
public abstract class CommandParser
{
   ...
   public static List<CommandArgument> parseArgs(String[] args)
         throws Exception
   {
   ...
   public static List<CommandArgument> parseFile(String filename)
         throws Exception
   {
   ...
   public static CommandArgument parseLine(int linenumber, String line)
         throws Exception
   {
      CommandArgument command = null;
      ...
      } else
      {
         ...
         for (int i = 1; i < args.length; i++)
         {
            ...
            if (arg.startsWith("-"))
            {
               ...
               if (key.equals("command"))
               {
                  throw new Exception("Line " + linenumber
                        + ": multiple command on one line is forbidden");
               } else
               {
                  if (parameters.containsKey(key))
                  {
                  ...
                  {
                     while ((i + 1 < args.length && !args[i + 1]
                           .startsWith("-")))
                     {
            ...
            {
               throw new Exception("Line " + linenumber
                     + ": unexpected argument outside a key : " + args[i]);
            }
         }
      }
   ...
   private static String[] splitLine(int linenumber, String line)
         throws Exception
   {
