...
public class Command extends AbstractCommand
{
    ...
    @Setter @Getter private EditObjectCommand editObjectCommand;

    @Setter @Getter private DaoCommand daoCommand;
    
    @Setter @Getter private ExchangeCommand exchangeCommand;
	...
	public void execute(String[] args)
	{
		...
		for (String key : globals.keySet())
		{
			log.info("global parameters "+key+" : "+ Arrays.toString(globals.get(key).toArray()));
		}
		...
		if (getBoolean(globals, "interactive"))
		{
			...
			while (true)
			{
				...
				if (!line.startsWith("#")) 
				{
					try 
					{
						CommandArgument command = CommandParser.parseLine(++commandNumber, line);
	...
	public List<NeptuneIdentifiedObject> executeCommand(
		...
		{
			log.info("    parameters "+key+" : "+ Arrays.toString(parameters.get(key).toArray()));
		}
