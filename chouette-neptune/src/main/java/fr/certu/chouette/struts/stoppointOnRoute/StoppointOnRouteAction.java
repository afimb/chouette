...
public class StoppointOnRouteAction extends GeneriqueAction implements ModelDriven<StopPoint>, Preparable
{
	...
	// Managers
	@Setter @Getter private INeptuneManager<Line> lineManager;
	@Setter @Getter private INeptuneManager<Route> routeManager;
	...
	private StopPoint networkModel = new StopPoint();
	...
	public void prepare() throws Exception
	{
		...
		arrets = stopPointManager.getAll(null, filter , level);
		//arretsModifies = ArretItineraire.dupliquer(arrets);
		arretsModifies = arrets;
		//Filter filter2 = Filter.getNewEqualsFilter("parentStopArea.id", )	
...
//	}
	...
	public Route getItineraire() throws ChouetteException
	{
		return routeManager.get(null, Filter.getNewEqualsFilter("id", idItineraire), level);
	}

	public Line getLigne() throws ChouetteException
	{
		return lineManager.get(null, Filter.getNewEqualsFilter("id", idLigne), level);
	}
