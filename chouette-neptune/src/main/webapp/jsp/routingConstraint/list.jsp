...
<s:url id="urlPositionGeographiques" action="list" namespace="/routingConstraint" includeParams="none">
  <s:param name="typePositionGeographique" value="%{typePositionGeographique}"/>
</s:url>
<title><s:text name="text.routingConstraint.list.title" /></title>
<s:property value="filAriane.addElementFilAriane(getText('text.routingConstraint.list.title'), '', #urlPositionGeographiques)"/>
<div class="panelData">
...
<%-- Filtre --%>
<div>
  <s:form action="list" namespace="/routingConstraint">
...
<%-- Actions --%>
<div class="actions">
  <s:url action="add" namespace="/routingConstraint" id="editPositionGeographique">
    <s:param name="typePositionGeographique" value="%{typePositionGeographique}" />
  </s:url>
  <s:a href="%{editPositionGeographique}"><b><s:text name="text.routingConstraint.create.button"/></b></s:a>
</div>
...
<div id="displaytag"> 
  <display:table name="positionGeographiques" pagesize="20" requestURI="" id="positionGeographique" export="false">
    <display:column titleKey="table.title.action" sortable="false">
      ...
      <%-- BOUTON SUPPRIMER --%>
      <s:url id="deletePositionGeographique" action="delete" namespace="/routingConstraint">
        <s:param name="idPositionGeographique">${positionGeographique.id}</s:param>
