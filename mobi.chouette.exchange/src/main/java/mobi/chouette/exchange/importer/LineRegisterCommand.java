...
if(importParameter.isKeepObsoleteLines() || isLineIsValidInFuture(newValue)) {
		log.info("register line : " + newValue.getCodeSpace() +  " " + newValue.getTechnicalId() + " " + newValue.getName() + " vehicleJourney count = "
				+ referential.getVehicleJourneys().size());
		try {

			optimiser.initialize(cache, referential);

			Line oldValue = cache.getLines().get(newValue.getChouetteId());
				...
				for (VehicleJourney item : referential.getVehicleJourneys().values()) {
					...
					for (VehicleJourneyAtStop vehicleJourneyAtStop : vehicleJourneyAtStops) {
						...
						StopPoint stopPoint = cache.getStopPoints().get(
								vehicleJourneyAtStop.getStopPoint().getChouetteId());
			...
			reporter.addObjectReport(context, newValue.getTechnicalId(), 
			...
			if (ex.getCause() != null) {
				...
				if (e instanceof SQLException) {
					...
					reporter.addErrorToObjectReport(context, newValue.getTechnicalId(), OBJECT_TYPE.LINE, ERROR_CODE.WRITE_ERROR,  e.getMessage());
					
				} else {
					reporter.addErrorToObjectReport(context, newValue.getTechnicalId(), OBJECT_TYPE.LINE, ERROR_CODE.INTERNAL_ERROR,  e.getMessage());
				}
			} else {
				reporter.addErrorToObjectReport(context, newValue.getTechnicalId(), OBJECT_TYPE.LINE, ERROR_CODE.INTERNAL_ERROR,  ex.getMessage());
			}
		}
		} else {
			log.info("skipping obsolete line : " + newValue.getCodeSpace() +  " " + newValue.getTechnicalId());
