...
public class RouteCheckPoints extends AbstractValidation<Route> implements Validator<Route> {
	...
	public ValidationConstraints validate(Context context, Route target) {
		...
		for (int i = 0; i < beans.size(); i++) {
			...
			if (test4_1)
				check4Generic1(report, route, L4_ROUTE_1, parameters, context, log);
	 ...
	 * @param areas
	 */
	private void check3Route1(ValidationReport report, Route route) {
		// 3-Route-1 : check if two successive stops are in same area
		...
		for (int j = 1; j < areas.size(); j++) {
			if (areas.get(j - 1).equals(areas.get(j))) {
				...
				Location targetLocation = new Location(areas.get(j));
	 ...
	 * @param areas
	 */
	private void check3Route2(ValidationReport report, Route route) {
		// 3-Route-2 : check if two wayback routes are actually waybacks
		List<StopArea> areas = NeptuneUtil.getStopAreaOfRoute(route);
		...
		if (route.getOppositeRoute() != null) {
			...
			if (!areasWb.isEmpty()) {
				...
				if (!first.equals(lastWb)) {
					target1 = new Location(first);
					target2 = new Location(lastWb);

				} else {
					target1 = new Location(firstWb);
					target2 = new Location(last);
	...
	private void check3Route3(ValidationReport report, Route route, ValidationParameters parameters) {
		List<StopArea> areas = NeptuneUtil.getStopAreaOfRoute(route);
		if (isEmpty(areas))
			return;
		// 3-Route-3 : check distance between stops
		...
		for (int i = 1; i < areas.size(); i++) {
			...
			if (distance < distanceMin) {
				Location location = new Location(route);
				Location target1 = new Location(firstArea);
				Location target2 = new Location(nextArea);
	...
	private void check3Route4(ValidationReport report, int rank, Route route, int rank2, Route route2) {
		// 3-Route-4 : check identical routes
		...
		if (!areas2.isEmpty()) {
			if (areas.equals(areas2)) {
				...
				Location location = new Location(route);
				Location target = new Location(route2);
	 ...
	 * @param route
	 */
	private void check3Route5(ValidationReport report, int rank, Route route, int rankWb, Route routeWb) {
		// 3-Route-5 : check for potentially waybacks
		if (route.getOppositeRoute() != null)
			return;
		List<StopArea> areas = NeptuneUtil.getStopAreaOfRoute(route);
		...
		if (!areasWb.isEmpty()) {
			...
			if (firstWb.equals(last) && lastWb.equals(first)) {
				...
				Location location = new Location(route);
				Location target = new Location(routeWb);
	 ...
	 * @param route
	 */
	private void check3Route6(ValidationReport report, Route route) {
		// 3-Route-6 : check if route has minimum 2 StopPoints
	 ...
	 * @param route
	 */
	private void check3Route7(ValidationReport report, Route route) {
		// 3-Route-7 : check if route has minimum 1 JourneyPattern
	 ...
	 * @param route
	 */
	private void check3Route8(ValidationReport report, Route route) {
		// 3-Route-8 : check if all stopPoints are used by journeyPatterns
		if (isEmpty(route.getJourneyPatterns()))
			return;
	 ...
	 * @param route
	 */
	private void check3Route9(ValidationReport report, Route route) {
		// 3-Route-9 : check if one journeyPattern uses all stopPoints
		if (isEmpty(route.getJourneyPatterns()))
			return;
