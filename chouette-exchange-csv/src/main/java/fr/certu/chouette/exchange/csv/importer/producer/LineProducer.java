...
public class LineProducer extends AbstractModelProducer<Line>
{
   ...
   @Override
   public Line produce(ChouetteCsvReader csvReader, String[] firstLine,
         String objectIdPrefix, String srid, Report report)
   {
      ...
      } else
      {
         ...
         CSVReportItem reportItem = new CSVReportItem(
               CSVReportItem.KEY.MANDATORY_TAG, Report.STATE.ERROR,
      ...
      try
      {
         line.setPublishedName(loadStringParam(csvReader,
               PUBLISHED_LINE_NAME_TITLE));
         line.setNumber(loadStringParam(csvReader, NUMBER_TITLE));
         if (line.getNumber().isEmpty())
         {
            throw new ExchangeException(ExchangeExceptionCode.MANDATORY_VALUE,
                  NUMBER_TITLE, "ligne");
         }
         {
            ...
            try
            {
               ...
               if (currentLine[TITLE_COLUMN].equals(CODE_TITLE))
               {
                  ...
                  if (line.getRegistrationNumber().isEmpty())
                  {
                     throw new ExchangeException(
                           ExchangeExceptionCode.MANDATORY_VALUE, CODE_TITLE,
                           "ligne");
                  }
               ...
               {
                  throw new ExchangeException(
                        ExchangeExceptionCode.MISSING_TAG, COMMENT_TITLE,
                        "ligne");
               }
            } catch (IOException e)
            {
               throw new ExchangeException(
                     ExchangeExceptionCode.INVALID_CSV_FILE, e);
            }
         }
         line.setTransportModeName(Utils.valueOfIgnoreCase(
               loadStringParam(csvReader, TRANSPORT_MODE_NAME_TITLE),
               TransportModeNameEnum.class));
         ...
         line.setObjectId(objectIdPrefix + ":" + Line.LINE_KEY + ":"
               + toIdString(line.getRegistrationNumber()));
         if (!NeptuneIdentifiedObject.checkObjectId(line.getObjectId()))
         {
            CSVReportItem reportItem = new CSVReportItem(
                  CSVReportItem.KEY.BAD_ID, Report.STATE.ERROR, line.getName(),
                  line.getObjectId());
            report.addItem(reportItem);
            return null;
         }
      ...
      } catch (ExchangeException e)
      {
         ...
         CSVReportItem reportItem = new CSVReportItem(
               CSVReportItem.KEY.INVALID_LINE, Report.STATE.ERROR,
               e.getLocalizedMessage());
         report.addItem(reportItem);
         return null;
      }
      // CSVReportItem reportItem = new
      // CSVReportItem(CSVReportItem.KEY.OK_LINE, Report.STATE.OK,
      // line.getName());
      // report.addItem(reportItem);
      return line;
   }

   private void loadRoutes(Line line, CSVReader csvReader,
         String objectIdPrefix, Report report) throws ExchangeException
   {
      try
      {
         ...
         if (checkLine(directions))
            throw new ExchangeException(ExchangeExceptionCode.MISSING_TAG,
                  DIRECTION_TITLE, lineName);
         if (!DIRECTION_TITLE.equals(directions[TITLE_COLUMN]))
            throw new ExchangeException(ExchangeExceptionCode.MISSING_TAG,
                  DIRECTION_TITLE, lineName);
         boolean hole = false;
         int waybackRouteColumn = TITLE_COLUMN + 1;
         int lastVehicleJourneyRank = directions.length; // last
         // VehicleJourney
         // rank (excluded)
         for (int i = TITLE_COLUMN + 1; i < directions.length; i++)
         {
            ...
            if (hole)
            {
               if (value != null)
                  throw new ExchangeException(
                        ExchangeExceptionCode.MISSING_VALUES, DIRECTION_TITLE,
            ...
            } else
            {
               if (value == null)
               {
                  if (i <= TITLE_COLUMN + 2)
                     throw new ExchangeException(
                           ExchangeExceptionCode.MISSING_VALUES,
                           DIRECTION_TITLE, Integer.toString(i), lineName);
                  else
                  {
                     ...
                     .equals(getValue(waybackRouteColumn, directions)))
               {
         ...
         if (checkLine(timetables))
            throw new ExchangeException(ExchangeExceptionCode.MISSING_TAG,
                  TIMETABLE_TITLE, lineName);
         if (!TIMETABLE_TITLE.equals(timetables[TITLE_COLUMN]))
            throw new ExchangeException(ExchangeExceptionCode.MISSING_TAG,
                  TIMETABLE_TITLE, lineName);
         if (timetables.length < lastVehicleJourneyRank)
            throw new ExchangeException(ExchangeExceptionCode.MISSING_VALUES,
                  TIMETABLE_TITLE, Integer.toString(timetables.length),
                  lineName);

         String[] specific = csvReader.readNext();
         if (checkLine(specific))
            throw new ExchangeException(ExchangeExceptionCode.MISSING_TAG,
                  SPECIFIC_TITLE, lineName);
         if (!SPECIFIC_TITLE.equals(specific[TITLE_COLUMN]))
            throw new ExchangeException(ExchangeExceptionCode.MISSING_TAG,
                  SPECIFIC_TITLE, lineName);
         if (specific.length < lastVehicleJourneyRank)
            throw new ExchangeException(ExchangeExceptionCode.MISSING_VALUES,
                  SPECIFIC_TITLE, Integer.toString(specific.length), lineName);
         String[] header = csvReader.readNext();
         if (checkLine(header))
            throw new ExchangeException(ExchangeExceptionCode.MISSING_TAG,
                  STOPHEADER_TITLE, lineName);
         ...
         while (!end)
         {
            ...
            if (arret.length < lastVehicleJourneyRank)
               throw new ExchangeException(
                     ExchangeExceptionCode.MISSING_VALUES, arret[TITLE_COLUMN],
         ...
         int wayBackRouteRank = 0;
         {
            ...
            PTDirectionEnum direction = Utils.valueOfIgnoreCase(
                  directions[routeColumn].substring(0, 1),
                  PTDirectionEnum.class);
            route.setDirection(direction);
            route.setWayBack(direction.toString());
            route.setObjectId(objectIdPrefix + ":" + Route.ROUTE_KEY + ":"
                  + toIdString(line.getRegistrationNumber()) + "_"
                  + route.getWayBack());
            if (!NeptuneIdentifiedObject.checkObjectId(route.getObjectId()))
            {
               CSVReportItem reportItem = new CSVReportItem(
                     CSVReportItem.KEY.BAD_ID, Report.STATE.ERROR,
                     route.getName(), route.getObjectId());
               report.addItem(reportItem);
            }
            // build stopPoint on route and stopArea (BP or Q)
            int rank = 1;
            String baseId = objectIdPrefix + ":" + StopPoint.STOPPOINT_KEY
                  + ":" + toIdString(line.getRegistrationNumber()) + "_"
                  + route.getWayBack() + "_";
            if ("00:00".equals(getValue(routeColumn, arrets.get(0)))
                  || "00:00:00".equals(getValue(routeColumn, arrets.get(0))))
               journeyColumn++;
            for (String[] a : arrets)
            {
            ...
            buildJourneys(route, arrets, timetables, specific, 0,
                  wayBackRouteRank, journeyColumn, waybackRouteColumn, lineName);
         }
         // build second route
         if (waybackRouteColumn != routeColumn)
         {
            ...
            PTDirectionEnum direction = Utils.valueOfIgnoreCase(
                  directions[waybackRouteColumn].substring(0, 1),
                  PTDirectionEnum.class);
            wayback.setDirection(direction);
            wayback.setWayBack(direction.toString());
            wayback.setObjectId(objectIdPrefix + ":" + Route.ROUTE_KEY + ":"
                  + toIdString(line.getRegistrationNumber()) + "_"
                  + wayback.getWayBack());
            if (!NeptuneIdentifiedObject.checkObjectId(wayback.getObjectId()))
            {
               CSVReportItem reportItem = new CSVReportItem(
                     CSVReportItem.KEY.BAD_ID, Report.STATE.ERROR,
                     wayback.getName(), wayback.getObjectId());
               report.addItem(reportItem);
            }
            ...
            if ("00:00".equals(getValue(waybackRouteColumn,
                  arrets.get(wayBackRouteRank)))
                  || "00:00:00".equals(getValue(waybackRouteColumn,
                        arrets.get(wayBackRouteRank))))
               journeyColumn++;
            ...
            buildJourneys(wayback, arrets, timetables, specific,
                  wayBackRouteRank, arrets.size(), journeyColumn,
    ...
    * @param endColumn
    */
   private void buildJourneys(Route route, List<String[]> arrets,
         String[] timetables, String[] specifics, int startRow, int endRow,
         int startColumn, int endColumn, String lineName)
         throws ExchangeException
   {
      ...
      for (int col = startColumn; col < endColumn; col++)
      {
         ...
         if (timetables[col] == null || timetables[col].isEmpty())
            throw new ExchangeException(ExchangeExceptionCode.MISSING_VALUES,
                  TIMETABLE_TITLE, Integer.toString(col), lineName);
         ...
         vj.setObjectId(route.getObjectId().replace(Route.ROUTE_KEY,
               VehicleJourney.VEHICLEJOURNEY_KEY)
               + "_" + rank);
         rank++;
         ...
         if (vj.getVehicleJourneyAtStops().size() > 0)
         {
            ...
            if (vj.checkJourneyPattern())
            {
               ...
               if (!journey.getObjectId().contains(":"))
               {
                  ...
                  journey.setObjectId(route.getObjectId().replace(
    ...
    * @return
    */
   private StopArea buildPhysical(String[] stopData, String objectIdPrefix,
         ChouetteAreaEnum areaType, Report report)
   {
      StopArea physical;
      physical = new StopArea();
      physical.setAreaType(areaType);
      physical.setName(getValue(STOPNAME_COLUMN, stopData));
      physical.setObjectId(objectIdPrefix + ":" + StopArea.STOPAREA_KEY
            + ":BP_" + toIdString(physical.getName()));
      ...
      if (physical.getLatitude() == null || physical.getLongitude() == null)
      {
         ...
         if (!physical.hasCoordinates())
         {
            ...
            CSVReportItem reportItem = new CSVReportItem(
                  CSVReportItem.KEY.STOP_WITHOUT_COORDS, Report.STATE.WARNING,
                  physical.getName());
            report.addItem(reportItem);
         }
      } else
      ...
      if (getValue(ADDRESS_COLUMN, stopData) != null
            || getValue(ZIPCODE_COLUMN, stopData) != null)
      {
         ...
         if (physical.getCountryCode() != null)
            physical.setObjectId(physical.getObjectId() + "_"
                  + toIdString(physical.getCountryCode()));
      }
      StopArea commercial = commercials
            .get(getValue(AREAZONE_COLUMN, stopData));
      if (commercial == null)
      {
      ...
      {
         CSVReportItem reportItem = new CSVReportItem(CSVReportItem.KEY.BAD_ID,
               Report.STATE.ERROR, physical.getName(), physical.getObjectId());
         report.addItem(reportItem);
      }
         ...
         Report report)
   {
      ...
      commercial.setObjectId(objectIdPrefix + ":" + StopArea.STOPAREA_KEY
            + ":C_" + toIdString(commercial.getName()));
      if (getValue(ADDRESS_COLUMN, stopData) != null
            || getValue(ZIPCODE_COLUMN, stopData) != null)
      {
         ...
         if (commercial.getCountryCode() != null)
            commercial.setObjectId(commercial.getObjectId() + "_"
                  + toIdString(commercial.getCountryCode()));
      }
      if (!NeptuneIdentifiedObject.checkObjectId(commercial.getObjectId()))
      {
         CSVReportItem reportItem = new CSVReportItem(CSVReportItem.KEY.BAD_ID,
               Report.STATE.ERROR, commercial.getName(),
