...
public class CompanyProducer extends AbstractModelProducer<Company>
{
   ...
   @Override
   public Company produce(ChouetteCsvReader csvReader, String[] firstLine,
         ...
         throws ExchangeException
   {
      ...
      try
      {
         ...
         company.setOrganisationalUnit(loadStringParam(csvReader,
               DESCRIPTION_TITLE));
         ...
         company.setObjectId(objectIdPrefix + ":" + Company.COMPANY_KEY + ":"
               + toIdString(company.getRegistrationNumber()));
         if (!NeptuneIdentifiedObject.checkObjectId(company.getObjectId()))
         {
            CSVReportItem reportItem = new CSVReportItem(
                  CSVReportItem.KEY.BAD_ID, Report.STATE.ERROR,
                  company.getName(), company.getObjectId());
            report.addItem(reportItem);
            return null;
         }
      ...
      CSVReportItem reportItem = new CSVReportItem(
            CSVReportItem.KEY.OK_COMPANY, Report.STATE.OK, company.getName());
      report.addItem(reportItem);
      return company;
   }
