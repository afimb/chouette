...
public class TimetableProducer extends AbstractModelProducer<Timetable>
{
   private static final Logger logger = Logger
         .getLogger(TimetableProducer.class);
   ...
   @Override
   public Timetable produce(ChouetteCsvReader csvReader, String[] firstLine,
         ...
         throws ExchangeException
   {
      ...
      } else
      {
         CSVReportItem reportItem = new CSVReportItem(
               ...
               firstLine[TITLE_COLUMN] + "<>" + TIMETABLE_LABEL_TITLE);
      ...
      try
      {
         ...
         timetable.setObjectId(objectIdPrefix + ":" + Timetable.TIMETABLE_KEY
         ...
         if (!NeptuneIdentifiedObject.checkObjectId(timetable.getObjectId()))
         {
            CSVReportItem reportItem = new CSVReportItem(
                  ...
                  timetable.getComment(), timetable.getObjectId());
            report.addItem(reportItem);
            return null;
         }
         if (loadStringParam(csvReader, MONDAY_TITLE).equalsIgnoreCase(
               YES_OPTION))
         {
            timetable.addDayType(DayTypeEnum.Monday);
         }
         if (loadStringParam(csvReader, TUESDAY_TITLE).equalsIgnoreCase(
               YES_OPTION))
         {
            timetable.addDayType(DayTypeEnum.Thursday);
         }
         if (loadStringParam(csvReader, WEDNESDAY_TITLE).equalsIgnoreCase(
               YES_OPTION))
         {
            timetable.addDayType(DayTypeEnum.Wednesday);
         }
         if (loadStringParam(csvReader, THURSDAY_TITLE).equalsIgnoreCase(
               YES_OPTION))
         {
            timetable.addDayType(DayTypeEnum.Thursday);
         }
         if (loadStringParam(csvReader, FRIDAY_TITLE).equalsIgnoreCase(
               YES_OPTION))
         {
            timetable.addDayType(DayTypeEnum.Friday);
         }
         if (loadStringParam(csvReader, SATURDAY_TITLE).equalsIgnoreCase(
               YES_OPTION))
         {
            timetable.addDayType(DayTypeEnum.Saturday);
         }
         if (loadStringParam(csvReader, SUNDAY_TITLE).equalsIgnoreCase(
               YES_OPTION))
         {
         ...
         if (startDates.size() != endDates.size())
         {
            ...
            CSVReportItem reportItem = new CSVReportItem(
                  CSVReportItem.KEY.BAD_TIMETABLE_PERIODS, Report.STATE.WARNING);
            report.addItem(reportItem);
         }
      ...
      CSVReportItem reportItem = new CSVReportItem(
            CSVReportItem.KEY.OK_TIMETABLE, Report.STATE.OK,
            timetable.getVersion(), timetable.getComment());
      report.addItem(reportItem);
      return timetable;
   }

   private List<Date> loadDatesParam(CSVReader csvReader, String title)
         throws ExchangeException
   {
