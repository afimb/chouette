...
public class PTNetworkProducer extends AbstractModelProducer<PTNetwork>
{
   ...
   private static final Logger logger = Logger
         .getLogger(PTNetworkProducer.class);
   ...
   @Override
   public PTNetwork produce(ChouetteCsvReader csvReader, String[] firstLine,
         ...
         throws ExchangeException
   {
      ...
      } else
      {
         CSVReportItem reportItem = new CSVReportItem(
               ...
               firstLine[TITLE_COLUMN] + "<>" + PTNETWORK_NAME_TITLE);
      ...
      {
         ptNetwork
               .setRegistrationNumber(loadStringParam(csvReader, CODE_TITLE));
         ptNetwork
               .setDescription(loadStringParam(csvReader, DESCRIPTION_TITLE));
         ptNetwork.setVersionDate(Calendar.getInstance().getTime());
         ptNetwork.setObjectId(objectIdPrefix + ":" + PTNetwork.PTNETWORK_KEY
               + ":" + ptNetwork.getRegistrationNumber());
         if (!NeptuneIdentifiedObject.checkObjectId(ptNetwork.getObjectId()))
         {
            CSVReportItem reportItem = new CSVReportItem(
                  CSVReportItem.KEY.BAD_ID, Report.STATE.ERROR,
                  ptNetwork.getName(), ptNetwork.getObjectId());
            report.addItem(reportItem);
            return null;
         }
      ...
      CSVReportItem reportItem = new CSVReportItem(
            CSVReportItem.KEY.OK_PTNETWORK, Report.STATE.OK,
            ptNetwork.getName(), ptNetwork.getDescription());
      report.addItem(reportItem);
      return ptNetwork;
   }
