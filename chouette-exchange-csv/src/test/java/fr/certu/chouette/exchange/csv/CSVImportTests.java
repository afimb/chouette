...
public class CSVImportTests extends AbstractTestNGSpringContextTests
{
   ...
   // @Test (groups = {"CheckParameters"}, description =
   // "Import Plugin should reject file not found",dependsOnMethods={"getBean"})
      ...
      for (Line line : lines)
      {
         ...
         for (Route route : line.getRoutes())
         {
            ...
            for (StopPoint point : route.getStopPoints())
            {
               ...
               Assert.assertNotNull(point.getContainedInStopArea().getParent(),
                     "StopAreas must have parent : "
   ...
   {
      Assert.assertEquals(tm.getObjectId(), "CSV:Timetable:" + tm.getVersion(),
            "Time Table must have correct objectId");
      Assert.assertNotNull(tm.getComment(), "Time Table must have comment");
      if (!tm.getPeriods().isEmpty())
      {
         Assert.assertEquals(tm.getPeriods().size(), 2,
               " when tm has periods, 2 periods are expected");
         int expectedDayType = Timetable.buildDayTypeMask(Arrays
               .asList(new DayTypeEnum[] { DayTypeEnum.Monday,
                     DayTypeEnum.Wednesday }));
         Assert.assertEquals(tm.getIntDayTypes(),
               Integer.valueOf(expectedDayType),
               "when tm has periods, day type expected for Monday and Wednesday");
      }
   ...
   private void assertArea(StopArea area, String prefix)
   {
      ...
      Assert.assertNotNull(area.getCountryCode(),
            "StopArea must have country code");
      String oid = prefix + area.getName() + "_" + area.getCountryCode();
      Assert.assertEquals(area.getObjectId(),
            "CSV:StopArea:" + toIdString(oid),
            "StopArea must have correct objectId");
   ...
   {
      Assert.assertEquals(network.getObjectId(), "CSV:GroupOfLine:1234",
            "network must have correct objectId");
      Assert.assertEquals(network.getName(), "Réseau 1",
            "network must have correct name");
      Assert.assertEquals(network.getDescription(), "Réseau de test CSV",
            "network must have correct comment");
      Assert.assertEquals(network.getRegistrationNumber(), "1234",
            "network must have correct registration number");
   }
   ...
   {
      Assert.assertEquals(company.getObjectId(), "CSV:Company:4321",
            "company must have correct objectId");
      Assert.assertEquals(company.getName(), "Transporteur 1",
            "company must have correct name");
      Assert.assertEquals(company.getShortName(), "un",
            "company must have correct short name");
      Assert.assertEquals(company.getCode(), "75000",
            "company must have correct zip code");
      Assert.assertEquals(company.getOrganisationalUnit(),
            "Transporteur de test CSV", "company must have correct Description");
      Assert.assertEquals(company.getPhone(), "01.02.03.04.05",
            "company must have correct phone number");
      Assert.assertEquals(company.getFax(), "01.03.05.07.09",
            "company must have correct fax number");
      Assert.assertEquals(company.getEmail(), "transport@transport.fr",
            "company must have correct email");
      Assert.assertEquals(company.getRegistrationNumber(), "4321",
            "company must have correct registration number");
   }
   ...
   {
      Assert.assertEquals(line.getObjectId(), "CSV:Line:ABCD" + rank,
            "line must have correct objectId");
      Assert.assertEquals(line.getName(), "Ligne CSV " + rank,
            "line must have correct name");
      Assert.assertEquals(line.getPublishedName(), "Ligne de test CSV " + rank,
            "line must have correct published name");
      Assert.assertEquals(line.getComment(),
            "Ligne de test pour le format CSV " + rank,
            "line must have correct comment");
      Assert.assertEquals(line.getRegistrationNumber(), "ABCD" + rank,
            "line must have correct registration number");
      Assert.assertEquals(line.getNumber(), "CSV " + rank,
            "line must have correct number");
      Assert.assertEquals(line.getTransportModeName(),
            TransportModeNameEnum.Bus, "line must be on Bus mode");
   ...
   private void assertRoute(Route route, int line_rank, int rank)
   {
      if (rank % 2 == 1)
      {
         Assert.assertEquals(route.getObjectId(), "CSV:Route:ABCD" + line_rank
               + "_A", "route must have correct objectId");
         Assert.assertEquals(route.getDirection(), PTDirectionEnum.A,
               "route must be on A direction");
         Assert.assertEquals(route.getWayBack(), "A",
               "route must be on A wayback");
         Assert.assertEquals(route.getName(), "ALLER",
               "route must have correct name");
         Assert.assertEquals(route.getWayBackRouteId(), "CSV:Route:ABCD"
               + line_rank + "_R", "route must have correct waybackId");
      } else
      {
         Assert.assertEquals(route.getObjectId(), "CSV:Route:ABCD" + line_rank
               + "_R", "route must have correct objectId");
         Assert.assertEquals(route.getDirection(), PTDirectionEnum.R,
               "route must be on R direction");
         Assert.assertEquals(route.getWayBack(), "R",
               "route must be on R wayback");
         Assert.assertEquals(route.getName(), "RETOUR",
               "route must have correct name");
         Assert.assertEquals(route.getWayBackRouteId(), "CSV:Route:ABCD"
               + line_rank + "_A", "route must have correct waybackId");
      }
   ...
   /**
	 * 
	 */
   private void assertJourneyPattern(JourneyPattern jp, int line_rank,
         int route_rank, String route_orientation, int rank)
   {
      Assert.assertEquals(jp.getObjectId(), "CSV:JourneyPattern:ABCD"
            + line_rank + "_" + route_orientation + "_" + rank,
            "journeyPattern must have correct objectId");
      ...
      Assert.assertEquals(jp.getStopPoints().size(), 4,
            "line journeyPattens must have 4 stopPoints");
      Assert.assertNotNull(jp.getVehicleJourneys(),
            "line journeyPattens must have vehicleJourneys");
      Assert.assertEquals(jp.getVehicleJourneys().size(), 2,
            "line journeyPattens must have 2 vehicleJourneys");
   }

   /**
	 * 
	 */
   private void assertVehicleJourney(VehicleJourney vj, int line_rank,
         int route_rank, String route_orientation, int rank)
   {
      Assert.assertEquals(vj.getObjectId(), "CSV:VehicleJourney:ABCD"
            + line_rank + "_" + route_orientation + "_" + rank,
            "vehicleJourney must have correct objectId");
      Assert.assertNotNull(vj.getVehicleJourneyAtStops(),
            "line vehicleJourneys must have vehicleJourneyAtStops");
      Assert.assertEquals(vj.getVehicleJourneyAtStops().size(), 4,
            "line vehicleJourneys must have 4 vehicleJourneyAtStops");
      Assert.assertNotNull(vj.getTimetables(),
            "line vehicleJourneys must have timetables");
      Assert.assertEquals(vj.getTimetables().size(), 1,
            "line vehicleJourneys must have 1 tiemtable");
   }

   /**
	 * 
	 */
   private void assertStopPoint(StopPoint stop, int line_rank, int route_rank,
         String route_orientation, int rank)
   {
      Assert.assertEquals(stop.getObjectId(), "CSV:StopPoint:ABCD" + line_rank
            + "_" + route_orientation + "_" + rank,
            "StopPoint must have correct objectId");
      Assert.assertNotNull(stop.getContainedInStopArea(),
            "StopPoint must have contained in StopArea");
   }
