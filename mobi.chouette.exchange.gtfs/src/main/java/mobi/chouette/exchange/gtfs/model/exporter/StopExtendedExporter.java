...
public class StopExtendedExporter extends ExporterImpl<GtfsStop> implements
		GtfsConverter {
	public static enum FIELDS {
		stop_id, stop_code, stop_name, stop_desc, stop_lat, stop_lon, zone_id, stop_url, location_type, parent_station, wheelchair_boarding, address_line, locality, postal_code, stop_timezone;
	};

	public static final String FILENAME = "stops.txt";

	public StopExtendedExporter(String name) throws IOException {
		super(name);
	}
	...
	public static Converter<String, GtfsStop> CONVERTER = new Converter<String, GtfsStop>() {
		...
		public GtfsStop from(Context context, String input) {
			...
			bean.setAddressLine(STRING_CONVERTER.from(context,
					FIELDS.address_line, values.get(i++), false));
			bean.setLocality(STRING_CONVERTER.from(context, FIELDS.locality,
					values.get(i++), false));
			bean.setPostalCode(STRING_CONVERTER.from(context,
					FIELDS.postal_code, values.get(i++), false));
		...
		public String to(Context context, GtfsStop input) {
			...
			values.add(STRING_CONVERTER.to(context, FIELDS.address_line,
					input.getAddressLine(), false));
			values.add(STRING_CONVERTER.to(context, FIELDS.locality,
	...
	public static class DefaultExporterFactory extends ExporterFactory {
		...
		protected Exporter create(String path) throws IOException {
			return new StopExtendedExporter(path);
		}
	}

	static {
		ExporterFactory factory = new DefaultExporterFactory();
		ExporterFactory.factories.put(StopExtendedExporter.class.getName(),
				factory);
	}
}
