...
public class StopTimeByTrip extends IndexImpl<GtfsStopTime> implements GtfsConverter {
	...
	protected void checkRequiredFields(Map<String, Integer> fields) {
		...
		if (fields.get(FIELDS.stop_id.name()) == null || fields.get(FIELDS.trip_id.name()) == null
				|| fields.get(FIELDS.departure_time.name()) == null || fields.get(FIELDS.arrival_time.name()) == null
				|| fields.get(FIELDS.stop_sequence.name()) == null) {

			if (fields.get(FIELDS.stop_id.name()) == null)
				throw new GtfsException(_path, 1, FIELDS.stop_id.name(), GtfsException.ERROR.MISSING_REQUIRED_FIELDS,
						null, null);
			if (fields.get(FIELDS.trip_id.name()) == null)
				throw new GtfsException(_path, 1, FIELDS.trip_id.name(), GtfsException.ERROR.MISSING_REQUIRED_FIELDS,
						null, null);
			if (fields.get(FIELDS.departure_time.name()) == null)
				throw new GtfsException(_path, 1, FIELDS.departure_time.name(),
						GtfsException.ERROR.MISSING_REQUIRED_FIELDS, null, null);
			if (fields.get(FIELDS.arrival_time.name()) == null)
				throw new GtfsException(_path, 1, FIELDS.arrival_time.name(),
						GtfsException.ERROR.MISSING_REQUIRED_FIELDS, null, null);
			if (fields.get(FIELDS.stop_sequence.name()) == null)
				throw new GtfsException(_path, 1, FIELDS.stop_sequence.name(),
						GtfsException.ERROR.MISSING_REQUIRED_FIELDS, null, null);
		}
	...
	protected GtfsStopTime build(GtfsIterator reader, Context context) {
		...
		if (value == null || value.trim().isEmpty()) {
			if (withValidation)
				_bean.getErrors().add(
						new GtfsException(_path, id, getIndex(FIELDS.stop_id.name()), FIELDS.stop_id.name(),
								GtfsException.ERROR.MISSING_REQUIRED_VALUES, null, null));
		} else {
		...
		if (value == null || value.trim().isEmpty()) {
			if (withValidation)
				_bean.getErrors().add(
						new GtfsException(_path, id, getIndex(FIELDS.stop_sequence.name()),
								FIELDS.stop_sequence.name(), GtfsException.ERROR.MISSING_REQUIRED_VALUES, null, null));
		} else {
			try {
				...
				if (stopSequence >= 0)
					...
					_bean.getErrors().add(
							new GtfsException(_path, id, getIndex(FIELDS.stop_sequence.name()), FIELDS.stop_sequence
		...
		if (value == null || value.trim().isEmpty()) {
			...
			try {
				...
				if (withValidation)
					_bean.getErrors().add(
							new GtfsException(_path, id, getIndex(FIELDS.arrival_time.name()), FIELDS.arrival_time
									.name(), GtfsException.ERROR.INVALID_FORMAT, null, value));
			}
		}
		...
		if (value == null || value.trim().isEmpty()) {
			...
			try {
				...
				if (withValidation)
					_bean.getErrors().add(
							new GtfsException(_path, id, getIndex(FIELDS.departure_time.name()), FIELDS.departure_time
									.name(), GtfsException.ERROR.INVALID_FORMAT, null, value));
			}
		}
		...
		if (noArrivalTime && !noDepartureTime) {
			_bean.getErrors().add(
					new GtfsException(_path, id, getIndex(FIELDS.arrival_time.name()), FIELDS.arrival_time.name(),
							GtfsException.ERROR.MISSING_ARRIVAL_TIME, null, null));
		} else if (!noArrivalTime && noDepartureTime) {
			if (withValidation)
				_bean.getErrors().add(
						new GtfsException(_path, id, getIndex(FIELDS.departure_time.name()), FIELDS.departure_time
								.name(), GtfsException.ERROR.MISSING_DEPARTURE_TIME, null, null));
		}
		...
		if (value != null && !value.trim().isEmpty()) {
			try {
				_bean.setPickupType(PICKUP_CONVERTER.from(context, FIELDS.pickup_type, value, PickupType.Scheduled,
						false));
			} catch (GtfsException e) {
				if (withValidation)
					_bean.getErrors().add(
							new GtfsException(_path, id, getIndex(FIELDS.pickup_type.name()),
									FIELDS.pickup_type.name(), GtfsException.ERROR.INVALID_FORMAT, null, value));
			}
		}
		...
		if (value != null && !value.trim().isEmpty()) {
			try {
				_bean.setDropOffType(DROPOFFTYPE_CONVERTER.from(context, FIELDS.drop_off_type, value,
						DropOffType.Scheduled, false));
			} catch (GtfsException e) {
				if (withValidation)
					_bean.getErrors().add(
							new GtfsException(_path, id, getIndex(FIELDS.drop_off_type.name()), FIELDS.drop_off_type
									.name(), GtfsException.ERROR.INVALID_FORMAT, null, value));
			}
		}
		...
		if (value != null && !value.trim().isEmpty()) {
			try {
				...
				if (shapeDistTraveled < 0) {
					if (withValidation)
						_bean.getErrors().add(
								new GtfsException(_path, id, getIndex(FIELDS.shape_dist_traveled.name()),
										FIELDS.shape_dist_traveled.name(), GtfsException.ERROR.INVALID_FORMAT, null,
										value));
				}
				...
				if (withValidation)
					_bean.getErrors()
							.add(new GtfsException(_path, id, getIndex(FIELDS.shape_dist_traveled.name()),
									FIELDS.shape_dist_traveled.name(), GtfsException.ERROR.INVALID_FORMAT, null, value));
			}
		}
		...
		if (value != null && !value.trim().isEmpty()) {
			try {
				_bean.setTimepoint(TIMEPOINTTYPE_CONVERTER.from(context, FIELDS.timepoint, value, null, false));
			} catch (GtfsException e) {
				if (withValidation)
					_bean.getErrors().add(
							new GtfsException(_path, id, getIndex(FIELDS.timepoint.name()), FIELDS.timepoint.name(),
	...
	public boolean validate(GtfsStopTime bean, GtfsImporter dao) {
		...
		if (isEmpty(stopId)) {
			...
			if (!dao.getStopById().containsKey(stopId)) {
				...
				bean.getErrors().add(
						new GtfsException(_path, bean.getId(), getIndex(FIELDS.stop_id.name()), FIELDS.stop_id.name(),
								GtfsException.ERROR.UNREFERENCED_ID, tripId, stopId));
				result2 = false;
			}
			if (result2)
