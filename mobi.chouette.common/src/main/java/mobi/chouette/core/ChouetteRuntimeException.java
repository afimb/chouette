...
public abstract class ChouetteRuntimeException extends RuntimeException {
	 ...
	 * <p/>
	 * populate message using Code for selection in internationalization
	 * properties file with arguments given from construction
	 * 
	 * @param locale
	 *            the linguistic and regional selection
	...
	public final String getLocalizedMessage(Locale locale) {
		try {
			...
			if (getCause() != null) {
				...
				message += "\n"
						+ MessageFormat.format(format, getCause()
								.getLocalizedMessage());
			}

			format = getFormat("message", locale);
			return MessageFormat
					.format(format, getPrefix(), getCode(), message);
	...
	/**
	 * get the generic format which combine prefix, code and specific message
	 * for final output
	 * <p/>
	 * if the entry is missing for the asked locale, it will be search with
	 * default locale before failure
	 * 
	 * @param key
	 *            the entry key in message properties
	 * @param locale
	 *            the required locale
	 * @return the format
	 */
	private String getFormat(String key, Locale locale) {
		try {
			ResourceBundle localBundle = ResourceBundle.getBundle(
					ChouetteException.class.getName(), locale);
			return localBundle.getString(key);
		} catch (MissingResourceException e) {
			ResourceBundle localBundle = ResourceBundle
					.getBundle(ChouetteException.class.getName());
			return localBundle.getString(key);
		}
	}
