...
public abstract class AbstractProducer {
	...
	static protected String toHubId(NeptuneIdentifiedObject neptuneObject) {
		if (neptuneObject == null || neptuneObject.getObjectId() == null)
			return null;
		String id;
		if (neptuneObject instanceof StopArea) {
			id = neptuneObject.getId().toString();
		} else {
			id = neptuneObject.objectIdSuffix();
		}
		return id;
	}
	...
	static protected Integer toHubTime(Time time) {
		...
		long seconds = ((calendar.get(Calendar.HOUR_OF_DAY) * 60) + calendar.get(Calendar.MINUTE)) * 60
	...
	static protected Integer toHubDelay(Time startTime, Time endTime) {
		...
		long startSeconds = ((startCalendar.get(Calendar.HOUR_OF_DAY) * 60) + startCalendar.get(Calendar.MINUTE)) * 60
				+ startCalendar.get(Calendar.SECOND);

		Calendar endCalendar = Calendar.getInstance();
		endCalendar.setTime(endTime);
		long endSeconds = ((endCalendar.get(Calendar.HOUR_OF_DAY) * 60) + endCalendar.get(Calendar.MINUTE)) * 60
