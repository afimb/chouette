...
public class GtfsCalendarDateFactory extends GtfsBeanFactory<GtfsCalendarDate>
{
   ...
   private static final Logger logger = Logger
         .getLogger(GtfsCalendarDateFactory.class);

   @Getter
   private final String dropSql = "drop table if exists calendardate;";
   @Getter
   private final String createSql = "create table calendardate (num, id, date,mode);";
   @Getter
   private final String createParentIndexSql = "create index calendardate_id_idx on calendardate (id)";
   private final String insertSQL = "insert into calendardate (num, id, date,mode) values (?, ?, ?, ?)";
   @Getter
   private final String selectSql = "select num, id, date,mode from calendardate ";
   @Getter
         ...
         Report report)
   {
      ...
      if (!bean.isValid())
      {
         ...
         {
            logger.warn("calendar_dates.txt : Line " + lineNumber
