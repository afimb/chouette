...
public class GtfsRouteFactory extends GtfsBeanFactory<GtfsRoute>
{
   ...
   @Getter
   private final String dropSql = "drop table if exists route;";
   @Getter
   private final String createSql = "create table route (num, id, shortname,longname,desc,type,agencyid,url,color,textcolor);";
   @Getter
   private final String createIndexSql = "create index route_id_idx on route (id)";
   private final String insertSQL = "insert into route (num, id, shortname,longname,desc,type,agencyid,url,color,textcolor) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
   @Getter
   private final String selectSql = "select num, id, shortname,longname,desc,type,agencyid,url,color,textcolor from route ";
   @Getter
   private final String[] dbHeader = new String[] { "num", "route_id",
         ...
         Report report)
   {
      ...
      if (!bean.isValid())
      {
         ...
         {
            logger.warn("routes.txt : Line " + lineNumber
