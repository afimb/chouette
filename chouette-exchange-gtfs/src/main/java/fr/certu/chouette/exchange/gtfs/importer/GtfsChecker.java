...
{
	public boolean check(GtfsImporter importer, ReportItem report, boolean full)
	{
		...
		if (full)
		{
			...
			try
			{
				...
				{
					report.addItem(new ExchangeReportItem(ExchangeReportItem.KEY.ZIP_ENTRY, Report.STATE.OK, fileName));
		...
		if (full)
		{
			...
			try
			{
				...
				ExchangeReportItem reportItem = new ExchangeReportItem(ExchangeReportItem.KEY.FILE_ERROR, Report.STATE.OK, fileName);
	...
	private boolean manageSystemException(String fileName, GtfsException e, boolean mandatory, Report report)
	{
		...
		if (e.getError().equals(GtfsException.ERROR.SYSTEM))
		{
			...
			{
				ReportItem item = new ExchangeReportItem(ExchangeReportItem.KEY.FILE_ERROR, Report.STATE.ERROR, fileName, "", e.getCause().getLocalizedMessage());
				report.addItem(item);
		...
		{
			ExchangeReportItem item = new ExchangeReportItem(ExchangeReportItem.KEY.FILE_ERROR, Report.STATE.ERROR, fileName);
			report.addItem(item);
			report.updateStatus(Report.STATE.ERROR);
		...
		else if (e.getError().equals(GtfsException.ERROR.INVALID_FILE_FORMAT))
		{
			ExchangeReportItem item = new ExchangeReportItem(ExchangeReportItem.KEY.FILE_ERROR, Report.STATE.ERROR, fileName);
			report.addItem(item);
			report.updateStatus(Report.STATE.ERROR);
			ExchangeReportItem detail = new ExchangeReportItem(ExchangeReportItem.KEY.INVALID_FORMAT, Report.STATE.ERROR, e.getId(), "Missing end of line");
			item.addItem(detail);
			log.error("zip import failed (cannot index " + fileName + ")" + e.toString());
			log.error("missing end of line : " + fileName);

		}
		else if (e.getError().equals(GtfsException.ERROR.DUPLICATE_FIELD))
		{
			ExchangeReportItem item = new ExchangeReportItem(ExchangeReportItem.KEY.FILE_ERROR, Report.STATE.ERROR, fileName);
			report.addItem(item);
			report.updateStatus(Report.STATE.ERROR);
			ExchangeReportItem detail = new ExchangeReportItem(ExchangeReportItem.KEY.DUPLICATE_ID, Report.STATE.ERROR, e.getId(), e.getField());
			item.addItem(detail);
			log.error("zip import failed (cannot index " + fileName + ")" + e.toString());
			log.error("dupplicate index key in : " + fileName);

		}
		else
		{
			ExchangeReportItem item = new ExchangeReportItem(ExchangeReportItem.KEY.FILE_ERROR, Report.STATE.ERROR, fileName, "", e.toString());
			report.addItem(item);
			report.updateStatus(Report.STATE.ERROR);
