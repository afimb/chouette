...
{
   /*
    * 0 or (empty) - This is a recommended transfer point between two routes. 1
    * - This is a timed transfer point between two routes. The departing vehicle
    * is expected to wait for the arriving one, with sufficient time for a
    * passenger to transfer between routes 2 - This transfer requires a minimum
    * amount of time between arrival and departure to ensure a connection. The
    * time required to transfer is specified by min_transfer_time. 3 - Transfers
    * are not possible between routes at this location.
    */
   public static enum Type
   {
      RECOMMENDED, TIMED, MINIMAL, FORBIDDEN
   };
   ...
   @Getter
   @Setter
   private GtfsTime minTransferTime = null;

   public String getCSVLine()
   {
      String csvLine = toCSVString(fromStopId) + "," + toCSVString(toStopId)
