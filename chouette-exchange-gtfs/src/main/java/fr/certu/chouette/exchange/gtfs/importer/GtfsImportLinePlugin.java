...
public class GtfsImportLinePlugin implements IImportPlugin<Line>
{
   private static final Logger logger = Logger
         .getLogger(GtfsImportLinePlugin.class);
   ...
   public GtfsImportLinePlugin()
   {
      ...
      {
         ParameterDescription param = new ParameterDescription(
               "mergeRouteByShortName", ParameterDescription.TYPE.BOOLEAN,
               false, "false");
         params.add(param);
      }
      {
      ...
      {
         ParameterDescription param = new ParameterDescription("colorFile",
      ...
      try
      {
               ...
               .hasMoreElements() && ok;)
         {
            ...
            if (entryName.endsWith("agency.txt"))
            {
               ...
               try
               {
                  ReportItem item = new LimitedExchangeReportItem(
                        ExchangeReportItem.KEY.ZIP_ENTRY, Report.STATE.OK,
                        entryName);
            ...
            } else if (entryName.endsWith("calendar.txt"))
            {
               ...
               } catch (Exception e)
               {
                  ...
                  report.addItem(item);
                  report.updateStatus(Report.STATE.ERROR);
                  logger.error("zip import failed (cannot read calendar.txt)"
                        + e.getLocalizedMessage(), e);
                  ok = false;
               }
            ...
            } else if (entryName.endsWith("stop_times.txt"))
            {
               ...
               } catch (Exception e)
               {
                  ...
                  report.addItem(item);
                  report.updateStatus(Report.STATE.ERROR);
                  logger.error("zip import failed (cannot read stop_times.txt)"
                        + e.getLocalizedMessage(), e);
                  ok = false;
               }
            ...
            // catch (Exception e)
            // {
            // logger.error("zip import failed (cannot read shapes.txt)" +
            // e.getLocalizedMessage(),e);
            // ok = false;
            // }
            // }
            else if (entryName.endsWith("transfers.txt"))
            {
               ...
               } catch (Exception e)
               {
                  ...
                  report.addItem(item);
                  report.updateStatus(Report.STATE.ERROR);
                  logger.error("zip import failed (cannot read transfers.txt)"
                        + e.getLocalizedMessage(), e);
                  ok = false;
               }
            } else
         ...
         if (!calendarFound)
         {
            ReportItem item = new ExchangeReportItem(
                  ExchangeReportItem.KEY.ZIP_MISSING_ENTRY, Report.STATE.ERROR,
                  "calendars.txt, calendar_dates.txt", filePath);
            report.addItem(item);
            report.updateStatus(Report.STATE.ERROR);
         ...
         if (ok && data.connect(report))
         {
            ...
            ModelAssembler assembler = converter.convert(optimizeMemory,
                  objectIdPrefix, incrementalPrefix, data,
                  maxDistanceForCommercialStop, ignoreLastWord,
                  ignoreEndCharacters, maxDistanceForConnectionLink,
                  mergeRouteByShortName, report);
            assembler.connect(report);
            if (colorPath != null)
               produceColorFile(colorPath, data, assembler);
   ...
   private void produceColorFile(String colorPath, GtfsData data,
