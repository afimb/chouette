...
public class GtfsData 
{
   ...
   public boolean connect()
   {
      ...
      //         GtfsRoute route = routeMap.get(trip.getRouteId());
      //         if (route == null)
      //         {
      //            logger.error("Trip line "+trip.getFileLineNumber()+" : tripId = "+trip.getTripId()+" has no route");
      //            ok = false;
      //         }
      //         else
      //         {
      //            route.addTrip(trip);
      //         }
      //         GtfsCalendar calendar = calendarMap.get(trip.getServiceId());
      //         if (calendar == null)
      //         {
      //            logger.error("Trip line "+trip.getFileLineNumber()+" : tripId = "+trip.getTripId()+" has no calendar");
      //            ok = false;
      //         }
      //         else
      //         {
      ...
      //         }
      //      }
      //      for (GtfsStopTime time : stopTimes)
      //      {
      //         GtfsTrip trip = tripMap.get(time.getTripId());
      //         if (trip == null)
      //         {
      //            logger.error("StopTime line "+time.getFileLineNumber()+" : tripId = "+time.getTripId()+",stopId = "+time.getStopId()+"stopSequence = "+time.getStopSequence()+" has no trip");
      //            ok = false; 
      //         }
      //         else
      //         {
      //            trip.addStopTime(time);
      //         }
      //         GtfsStop stop = stopMap.get(time.getStopId());
      //         if (stop == null)
      //         {
      //            logger.error("StopTime line "+time.getFileLineNumber()+" : tripId = "+time.getTripId()+",stopId = "+time.getStopId()+"stopSequence = "+time.getStopSequence()+" has no stop");
      //            ok = false;
      //         }
      //         else
      //         {
      ...
      //      {
      //         GtfsTrip trip = tripMap.get(frequency.getTripId());
      //         if (trip == null)
      //         {
      //            logger.error("Frequency line "+frequency.getFileLineNumber()+" : tripId = "+frequency.getTripId()+" has no trip");
      //            ok = false;
      //         }
      //         else
      //         {
      //            trip.addFrequency(frequency);
      //         }
      //      }
      //      for (GtfsTrip trip : trips)
      //      {
      //         List<GtfsStopTime> times = trip.getStopTimes();
      //         if (times.isEmpty())
      //         {
      //            logger.error("Trip line "+trip.getFileLineNumber()+" : tripId = "+trip.getServiceId()+" has no stoptimes");
      //            ok = false;
      //         }
      //         else
      //         {
      //            GtfsStopTime previous = times.get(0);
      //            if (previous.getArrivalTime().compareTo(previous.getDepartureTime()) > 0)
      //            {
      //               logger.error("StopTime line "+previous.getFileLineNumber()+" : tripId = "+previous.getTripId()+" ,stopId = "+previous.getStopId()+" ,stopSequence = "+previous.getStopSequence()+" has arrivalTime after departureTime");
      //               ok = false;
      //            }
      //            for (int i = 1; i < times.size(); i++)
      //            {
      //               GtfsStopTime next = times.get(i);
      //               if (next.getArrivalTime().compareTo(next.getDepartureTime()) > 0)
      //               {
      //                  logger.error("StopTime line "+next.getFileLineNumber()+" : tripId = "+next.getTripId()+" ,stopId = "+next.getStopId()+" ,stopSequence = "+next.getStopSequence()+" has arrivalTime after departureTime");
      //                  ok = false;
      //               }
      //               if (next.getArrivalTime().compareTo(previous.getDepartureTime()) < 0)
      //               {
      //                  logger.error("StopTime line "+next.getFileLineNumber()+" : tripId = "+next.getTripId()+" ,stopId = "+next.getStopId()+", stopSequence = "+next.getStopSequence()+" has arrivalTime before previous stopTime (line "+previous.getFileLineNumber()+") departureTime");
      //                  ok = false;
      //               }
      //               previous = next;
      //            }
