...
public class GtfsAgencyFactory extends GtfsBeanFactory<GtfsAgency>
{
   ...
   private static final Logger logger = Logger
         .getLogger(GtfsAgencyFactory.class);

   @Getter
   private final String dropSql = "drop table if exists agency;";
   @Getter
   private final String createSql = "create table agency (num, id, name,url,timezone,lang,phone);";
   private final String insertSQL = "insert into agency (num, id, name,url,timezone,lang,phone) values (?, ?, ?, ?, ?, ?, ?)";
   @Getter
   private final String selectSql = "select num, id, name,url,timezone,lang,phone from agency ";
   @Getter
   private final String createIndexSql = "create index agency_id_idx on agency (id)";
   @Getter
   private final String[] dbHeader = new String[] { "num", "agency_id",
    ...
    * 
    * @see
    * fr.certu.chouette.exchange.gtfs.model.factory.GtfsBeanFactory#getNewGtfsBean
         ...
         Report report)
   {
      ...
      if (!bean.isValid())
      {
         ...
         {
            logger.warn("agency.txt : Line " + lineNumber
