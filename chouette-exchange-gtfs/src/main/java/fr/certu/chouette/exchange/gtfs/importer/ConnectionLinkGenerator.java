...
public class ConnectionLinkGenerator extends AbstractGenerator
{
   private static final Logger logger = Logger
         .getLogger(ConnectionLinkGenerator.class);

   public List<ConnectionLink> createConnectionLinks(
         List<StopArea> commercials, double distanceMax,
         List<ConnectionLink> fixedLinks, List<ConnectionLink> excludedLinks)
   {
      double minDistanceRejected = 1000000;
      Map<String, ConnectionLink> fixedLinkMap = ConnectionLink
            .mapOnObjectIds(fixedLinks);
      Map<String, ConnectionLink> excludedLinkMap = ConnectionLink
            .mapOnObjectIds(excludedLinks);
      ...
      for (int i = 0; i < commercials.size() - 1; i++)
      {
         ...
         for (int j = i + 1; j < commercials.size(); j++)
         {
            ...
            double distance = distance(source.getLongitude().doubleValue(),
                  source.getLatitude().doubleValue(), target.getLongitude()
                        .doubleValue(), target.getLatitude().doubleValue());
            if (distance < distanceMax)
            {
               // eligible, check route
               List<String> sourceRoutes = routesForStopArea.get(source
                     .getObjectId());
               ...
               List<String> targetRoutes = routesForStopArea.get(target
                     .getObjectId());
               ...
               if (ok)
               {
                  ...
                  if (fixedLinkMap.containsKey(objectId)
                        || fixedLinkMap.containsKey(reverseId))
                  {
                     ...
                     if (link != null)
                     {
                        ...
                        logger.info("ConnectionLink " + link.getName()
                              + " updated");
                     }
                  ...
                  } else
                  {
                     ...
                     link.setName("from " + source.getName() + " to "
                           + target.getName());
                     ...
                     objectId = sourceToken[0] + ":"
                           + ConnectionLink.CONNECTIONLINK_KEY + ":"
                           + targetToken[2] + "_" + sourceToken[2];
                     reverseLink.setObjectId(objectId);
                     reverseLink.setCreationTime(Calendar.getInstance()
                           .getTime());
                     ...
                     reverseLink.setName("from " + target.getName() + " to "
                           + source.getName());
      ...
      if (links.isEmpty())
      {
         logger.info("ConnectionLink : no links builded , minimal distance found = "
               + minDistanceRejected + " > " + distanceMax);
      }
      return links;
   }
