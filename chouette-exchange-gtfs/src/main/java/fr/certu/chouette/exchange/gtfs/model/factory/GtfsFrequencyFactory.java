...
public class GtfsFrequencyFactory extends GtfsBeanFactory<GtfsFrequency>
{
   ...
   @Getter
   private final String dropSql = "drop table if exists frequency;";
   @Getter
   private final String createSql = "create table frequency (num, tripid, starttime,endtime,headwaysecs,exacttimes);";
   @Getter
   private final String createParentIndexSql = "create index frequency_tripid_idx on frequency (tripid)";
   private final String insertSQL = "insert into frequency (num, tripid, starttime,endtime,headwaysecs,exacttimes) values (?, ?, ?, ?, ?, ?)";
   @Getter
   private final String selectSql = "select num, tripid, starttime,endtime,headwaysecs,exacttimes from frequency ";
   @Getter
         ...
         Report report)
   {
      ...
      if (!bean.isValid())
      {
         ...
         {
            logger.warn("frequencies.txt : Line " + lineNumber
