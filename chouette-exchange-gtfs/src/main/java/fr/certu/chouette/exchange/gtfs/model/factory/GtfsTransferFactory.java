...
public class GtfsTransferFactory extends GtfsBeanFactory<GtfsTransfer>
{
   ...
   private static final Logger logger = Logger
         .getLogger(GtfsTransferFactory.class);
   @Getter
   private final String dropSql = "drop table if exists transfer;";
   @Getter
   private final String createSql = "create table transfer (num, fromstopid, tostopid,transfertype,mintransfertime);";
   private final String insertSQL = "insert into transfer (num, fromstopid, tostopid,transfertype,mintransfertime) values (?, ?, ?, ?, ?)";
   @Getter
   private final String selectSql = "select num, fromstopid, tostopid,transfertype,mintransfertime from transfer ";
   @Getter
   private final String[] dbHeader = new String[] { "num", "from_stop_id",
         "to_stop_id", "transfer_type", "min_transfer_time" };

   public GtfsTransferFactory()
   {
      super(GtfsTransfer.class);
   }
   ...
   public GtfsTransfer getNewGtfsBean(int lineNumber, String[] csvLine,
         Report report)
   {
      GtfsTransfer bean = getNewGtfsBean(GtfsTransfer.class);
      ...
      {
      case 1:
         bean.setTransferType(GtfsTransfer.Type.TIMED);
         break;
      case 2:
         bean.setTransferType(GtfsTransfer.Type.MINIMAL);
         break;

      case 3:
         bean.setTransferType(GtfsTransfer.Type.FORBIDDEN);
         break;

      default:
         bean.setTransferType(GtfsTransfer.Type.RECOMMENDED);
      ...
      if (!bean.isValid())
      {
         ...
         {
            logger.warn("transfers.txt : Line " + lineNumber
   ...
   public void saveAll(Connection conn, List<GtfsTransfer> beans)
   {
      ...
      try
      {
         ...
         for (GtfsTransfer bean : beans)
         {
            ...
            setStringOrNull(prep, 4,
                  Integer.toString(bean.getTransferType().ordinal()));
