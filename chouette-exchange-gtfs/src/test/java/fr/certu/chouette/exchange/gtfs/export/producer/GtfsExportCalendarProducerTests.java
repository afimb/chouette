...
{
	private static final Logger logger = Logger.getLogger(GtfsExportCalendarProducerTests.class);
	...
	public void verifyCalendarProducer2() throws ChouetteException 
	{
		...
		Assert.assertFalse(gtfsObject.isSunday(),"sunday must be false");
		Assert.assertEquals(gtfsObject.getCalendarDates().size(), 5, "calendar must have 5 dates");
		c.set(Calendar.YEAR, 2013);
		c.set(Calendar.MONTH, Calendar.JULY);
		c.set(Calendar.DAY_OF_MONTH, 1);
	...
	public void verifyCalendarProducer5() throws ChouetteException 
	{
		...
		Assert.assertEquals(gtfsObject.getServiceId(), toGtfsId(neptuneObject.getObjectId()),"service id must be correcty set");
		Assert.assertNull(gtfsObject.getStartDate(), "start date must be correcty set");
		Assert.assertNull(gtfsObject.getEndDate(), "end date must be correcty set");
		Assert.assertTrue(gtfsObject.isMonday(),"monday must be true");
		Assert.assertTrue(gtfsObject.isTuesday(),"tuesday must be true");
		Assert.assertFalse(gtfsObject.isWednesday(),"wednesday must be false");
		Assert.assertTrue(gtfsObject.isThursday(),"thursday must be true");
		...
		Assert.assertTrue(gtfsObject.isSaturday(),"saturday must be true");
		Assert.assertTrue(gtfsObject.isSunday(),"sunday must be true");
		Assert.assertEquals(gtfsObject.getCalendarDates().size(), 28, "calendar must have 28 dates");
		c.set(Calendar.YEAR, 2013);
		c.set(Calendar.MONTH, Calendar.JULY);
		c.set(Calendar.DAY_OF_MONTH, 1);
		if (c.get(Calendar.DAY_OF_WEEK) == Calendar.WEDNESDAY) c.add(Calendar.DATE, 1);
