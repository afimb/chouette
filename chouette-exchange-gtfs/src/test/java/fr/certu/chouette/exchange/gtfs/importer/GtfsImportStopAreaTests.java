...
{
	private static final Logger logger = Logger.getLogger(GtfsImportStopAreaTests.class);
	...
	public void getBean()
	{
		importStopArea = (IImportPlugin<StopArea>) applicationContext.getBean("GtfsStopAreaImport") ;
	}
	...
	private List<StopArea> verifyImportStopArea(String file,int areaSize, int connectionLinkSize) throws ChouetteException
	{
		List<ParameterValue> parameters = new ArrayList<ParameterValue>();
		SimpleParameterValue simpleParameterValue = new SimpleParameterValue("inputFile");
		...
		Assert.assertEquals(areas.size(), areaSize,"areas size must equals "+areaSize);
		...
		Assert.assertEquals(clinks.size(),connectionLinkSize,"connection links size must equals "+connectionLinkSize);
