...
public class TransitDataStatisticsService {
     ...
     *                                  is from 0 to lowest key value. Corresponding value in map is used as categegory name,
     * @return
    ...
    void categorizeValidity(LineStatistics lineStats, Date startDate, Map<Integer, String> minDaysValidityCategories) {
        ...
        for (Integer numDays : categories) {
            if (numDays>0) {
                ValidityCategory category = new ValidityCategory(getCategoryName(minDaysValidityCategories, numDays, numDays.toString()), numDays, new ArrayList<>());
