...
@Data
@Log4j
public class GeojsonExporterProcessingCommands implements ProcessingCommands,
		Constant {
	...
	public static class DefaultFactory extends ProcessingCommandsFactory {
		...
		protected ProcessingCommands create() throws IOException {
			ProcessingCommands result = new GeojsonExporterProcessingCommands();
			return result;
		}
	}

	static {
		ProcessingCommandsFactory.factories.put(
				GeojsonExporterProcessingCommands.class.getName(),
				new DefaultFactory());
	}
	...
	public List<? extends Command> getPreProcessingCommands(Context context,
		...
		try {
			commands.add(CommandFactory.create(initCtx,
					GeojsonInitExportCommand.class.getName()));
		} catch (Exception e) {
			log.error(e, e);
	...
	public List<? extends Command> getPostProcessingCommands(Context context,
		...
		InitialContext initialContext = (InitialContext) context
				.get(INITIAL_CONTEXT);
		GeojsonExportParameters parameters = (GeojsonExportParameters) context
				.get(CONFIGURATION);
		List<Command> commands = new ArrayList<>();
		try {
			commands.add(CommandFactory.create(initialContext,
	...
	@Override
	public List<? extends Command> getStopAreaProcessingCommands(
			Context context, boolean withDao) {
		return new ArrayList<>();
	}

	@Override
	public List<? extends Command> getDisposeCommands(Context context,
			boolean withDao) {
		List<Command> commands = new ArrayList<>();
		return commands;
	}
