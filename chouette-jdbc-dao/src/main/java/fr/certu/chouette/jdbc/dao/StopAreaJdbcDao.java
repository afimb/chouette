...
public class StopAreaJdbcDao extends AbstractJdbcDao<StopArea>
{
         ...
         throws SQLException
   {
      ...
      {
         ps.setNull(1, Types.BIGINT);
         logger.warn("stoparea " + stopArea.getObjectId() + " "
               + stopArea.getName() + " has unsaved parent ");
         logger.warn("parent = " + stopArea.getParent().getObjectId() + " "
               + stopArea.getParent().getName());
      } else
      {
      ...
      ps.setBoolean(17, stopArea.isLiftAvailable());
         ...
         PreparedStatement ps, Object attribute) throws SQLException
   {
      ...
      if (attributeKey.equals("stopAreaStopArea"))
      {
         ...
         logger.debug("save relation parentId = " + child.parentId
      ...
      if (attributeKey.equals("routingconstraints"))
      {
         ...
         logger.debug("save relation line = " + routing.lineId
               + ", stopArea = " + routing.stopAreaId);
   ...
   protected Collection<? extends Object> getAttributeValues(
         String attributeKey, StopArea item)
   {
      if (attributeKey.equals("stopAreaStopArea"))
      {
         ...
         if (item.getRoutingConstraintAreas() != null)
         {
            for (StopArea child : item.getRoutingConstraintAreas())
            {
               ...
               logger.debug("prepare relation parentId = " + object.parentId
      ...
      if (attributeKey.equals("routingconstraints"))
      {
         ...
         if (item.getRoutingConstraintLines() != null)
         {
            for (Line routing : item.getRoutingConstraintLines())
            {
               ...
               logger.debug("prepare relation line = " + object.lineId
                     + ", stopArea = " + object.stopAreaId);
    ...
    * 
    * @see
    * fr.certu.chouette.jdbc.dao.AbstractJdbcDao#afterSaveOrUpdateAllProcessing
    * (java.util.List)
    */
   // @Override
