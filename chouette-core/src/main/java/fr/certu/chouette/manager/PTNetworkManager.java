...
public class PTNetworkManager extends AbstractNeptuneManager<PTNetwork>
{
   private static final Logger logger = Logger
         .getLogger(PTNetworkManager.class);
   ...
   @Transactional
   @Override
   public void remove(User user, PTNetwork ptNetwork, boolean propagate)
         throws ChouetteException
   {
      INeptuneManager<Line> lineManager = (INeptuneManager<Line>) getManager(Line.class);
      List<Line> lines = lineManager.getAll(user,
            Filter.getNewEqualsFilter("ptNetwork.id", ptNetwork.getId()));
      if (propagate)
