 ...
 * <p/>
 * an easy Database request builder similar to Hibernate Criterion but which can
 * be use with oder DAO access
 * 
 */
public class Filter
{
   ...
   {
      EQUALS, NOT_EQUALS, LESS, LESS_OR_EQUALS, GREATER, GREATER_OR_EQUALS, LIKE, ILIKE, IN, AND, OR, NOT, IS_NULL, SQL_WHERE, BETWEEN
   }
    ...
    */
   @Getter
   private TypeFamilly familly;
   /**
    * Filter target bean attribute : name of the bean attribute
    * <p/>
    * when a bean contains dependency with another bean, the filter can point on
    * an attribute of the dependency bean <br/>
    ...
    * </pre>
    * 
    * where <i>ptNetwork</i> is the network attribute name in line bean and
    * <i>name</i> is the name attribute in PTNework bean
    */
   @Getter
   private String attribute;
   /**
    * required value for a TERMINAL family filter (first value when the filter
    * requires 2 values
    */
   @Getter
   ...
   /**
    * list of required values for a TERMINAL family filter (which requires
    * muliple values
    * <p/>
   ...
   /**
    * required value for a COMBINED family filter of first one for filter which
    * requires 2 values
    */
   @Getter
   private Filter[] combinedFilters = null;
   /**
    ...
    */
   @Getter
   private List<FilterOrder> orderList = null;
   /**
   ...
   private int start = 0;
   /**
    * limit discriminant for deepness result management (0 will be ignored ,
    * i.e. no limit)
    */
   @Getter
   private int limit = 0;
   ...
   /**
    * private constructor, must use the static methods
    * getNew<i>Type</i>Filter(...)
    * <p/>
    ...
    * getNew<i>Type</i>Filter(...)
    * <p/>
    * internal use for EQUALS, NOT_EQUALS, LESS, LESS_OR_EQUALS, GREATER,
    * GREATER_OR_EQUALS, LIKE , ILIKE and IN filters
    * 
    * @param type
    *           filter type
    * @param attribute
   ...
   private Filter(Type type, String attribute, Object value)
   {
      ...
      if (type.ordinal() > Type.IN.ordinal())
      {
         throw new IllegalArgumentException(
               "This constructor accept only EQUALS, NOT_EQUALS, LESS, LESS_OR_EQUALS, GREATER, GREATER_OR_EQUALS, LIKE , ILIKE and IN types");
      }
      ...
      if (type.ordinal() >= Type.LESS.ordinal()
            && type.ordinal() <= Type.GREATER_OR_EQUALS.ordinal())
      {
         if (!(value instanceof Number) && !(value instanceof Date))
         {
            throw new IllegalArgumentException(
                  "LESS, LESS_OR_EQUALS, GREATER, GREATER_OR_EQUALS accept only numeric or date types");
         }
      ...
      if (type.equals(Type.IN))
      {
         ...
         {
            throw new IllegalArgumentException(
                  "IN accept only a sub select clause in a string");
         }
      }
   }
    ...
    * 
    * @param attribute
    *           the bean attribute name concerned
    * @param value
    ...
    */
   public static Filter getNewGreaterOrEqualsFilter(String attribute,
         Object value)
   {
    ...
    */
   public static Filter getNewIgnoreCaseLikeFilter(String attribute,
         String value)
   {
   ...
   private Filter(Type type, String attribute, Object value1, Object value2)
   {
      ...
      {
         throw new IllegalArgumentException(
               "This constructor accept only BETWEEN type");
      }
      ...
      {
         throw new IllegalArgumentException(
               "BETWEEN accept only numeric or date types");
      }
      ...
      {
         throw new IllegalArgumentException(
               "BETWEEN accept only identical value types");
      }
    ...
    *           the bean attribute name concerned
    * @param value1
    *           the lower value to be compared
    * @param value2
    *           the upper value to be compared
    * @return a Between filter
    */
   public static Filter getNewBetweenFilter(String attribute, Object value1,
         Object value2)
   {
   ...
   private Filter(Type type, String attribute, Object[] valueArray)
   {
      ...
      {
         throw new IllegalArgumentException(
               "This constructor accept only IN type");
      }
   }
    ...
    *           filter type
    * @param filters
    *           array of criteria to combine
    */
   private Filter(Type type, Filter... filters)
   {
      this();
      if (type.ordinal() < Type.AND.ordinal()
            || type.ordinal() > Type.OR.ordinal())
      {
         throw new IllegalArgumentException(
               "This constructor accept only AND and OR types");
      }
      ...
      for (Filter filter : filters)
      {
         if (filter == null)
            continue;
         if (filter.orderList != null)
            addAllOrder(filter.orderList);
         this.limit = Math.max(this.limit, filter.limit);
         this.start = Math.min(this.start, filter.start);
      }
      if (realFilters.size() == 0)
         return; // no usefull filters create empty filter
   ...
   {
      this();
      if (first.getFamilly().equals(TypeFamilly.EMPTY))
         return;
      if (type.equals(Type.NOT))
      {
         ...
         if (first.orderList != null)
            this.addAllOrder(first.orderList);
         this.limit = first.limit;
      } else
      {
         throw new IllegalArgumentException(
               "This constructor accept only NOT type");
      }
   }
   ...
   private Filter(Type type, String attribute)
   {
      ...
      {
         throw new IllegalArgumentException(
               "This constructor accept only IS_NULL and SQL_WHERE types");
      }
   }
    ...
    * 
    * @param sqlQuery
    *           the literal query (must conform mapping property files)
   ...
   {
      if (startRestriction < 0)
         throw new IllegalArgumentException(
               "startRestriction can not be negative");
   ...
   {
      if (limitRestriction < 0)
         throw new IllegalArgumentException(
               "limitRestriction can not be negative");
