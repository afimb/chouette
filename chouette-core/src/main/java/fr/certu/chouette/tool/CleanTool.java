...
public class CleanTool implements ICleanTool
{
	...
	@Setter private INeptuneManager<JourneyPattern> journeyPatternManager; 
	...
	public Report purgeAllItems(Date boundaryDate, boolean before) 
	{
		...
		try 
		{
			...
			Filter physicalStopFilter = Filter.getNewInFilter(StopArea.AREA_TYPE, new Object[]{ChouetteAreaEnum.BOARDINGPOSITION,ChouetteAreaEnum.QUAY});
			...
			Filter commercialStopFilter = Filter.getNewEqualsFilter(StopArea.AREA_TYPE, ChouetteAreaEnum.COMMERCIALSTOPPOINT);
			...
			//delete JourneyPatterns which aren't referenced in vehicleJourney table
			...
			purgeReport.addItem(new CleanReportItem("VehicleJourneyCount",Long.toString(numberOfVehicleJourneyBeforePurge),Long.toString(numberOfVehicleJourneyAfterPurge)));
			purgeReport.addItem(new CleanReportItem("PhysicalStopCount",Long.toString(numberOfPhysicalStopPointBeforePurge),Long.toString(numberOfPhysicalStopPointAfterPurge)));
			purgeReport.addItem(new CleanReportItem("CommercialStopCount",Long.toString(numberOfCommercialStopPointBeforePurge),Long.toString(numberOfCommercialStopPointAfterPurge)));
			purgeReport.addItem(new CleanReportItem("ConnectionLinkCount",Long.toString(numberOfConnectionLinkBeforePurge),Long.toString(numberOfConnectionLinkAfterPurge)));
	...
	{
		if (before) return period.getEndDate().before(boundaryDate);
	...
	{
		if (before) return date.before(boundaryDate);
