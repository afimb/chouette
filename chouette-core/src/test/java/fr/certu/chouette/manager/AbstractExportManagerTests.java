...
public abstract class AbstractExportManagerTests<T extends NeptuneIdentifiedObject>
      extends AbstractManagerTests<T>
{
   ...
   public void initManager(String beanName, String managerName, T bean)
   {
      ...
      exportDeletionDescription = new FormatDescription(this.getClass()
            .getName());
   ...
   {
      expect(exportMock.getDescription()).andReturn(exportDescription)
            .anyTimes();
      replay(exportMock);
      manager.addExportPlugin(exportMock);
      List<FormatDescription> list = manager.getExportFormats(null);
      Assert.assertTrue(list.contains(exportDescription),
            "list should contain expected format");
      verify(exportMock);
   }
   ...
   public void verifyDoExport() throws ChouetteException
   {
      ...
      manager.addExportPlugin(exportMock);
      manager
            .doExport(null, beans, exportDescription.getName(), values, report);
      verify(exportMock);
   }
