...
public abstract class AbstractDaoManagerTests<T extends NeptuneIdentifiedObject>
      extends AbstractManagerTests<T>
{
   ...
   public void verifyGetWithId() throws ChouetteException
   {
      ...
      replay(daoMock);
      T retBean = manager.get(null,
            Filter.getNewEqualsFilter("id", bean.getId()));
      Assert.assertTrue(retBean != null, "bean should not be null");
      Assert.assertTrue(retBean.getId() == bean.getId(),
            "bean should have expected id");
      verify(daoMock);
   }
   ...
   public void verifyGetWithObjectId() throws ChouetteException
   {
      ...
      replay(daoMock);
      T retBean = manager.get(null,
            Filter.getNewEqualsFilter("objectId", bean.getObjectId()));
      Assert.assertTrue(retBean != null, "bean should not be null");
      Assert.assertTrue(retBean.getId() == bean.getId(),
            "bean should have expected objectid");
      verify(daoMock);
   }
   ...
   public void verifyGetIdNotFound() throws ChouetteException
   {
      ...
      {
         manager.setDao(daoMock);
         expect(daoMock.get(bean.getId())).andThrow(
               new DummyRuntimeException("no such id"));
         replay(daoMock);
   ...
   public void verifyGetObjectIdNotFound() throws ChouetteException
   {
      ...
      {
         manager.setDao(daoMock);
         expect(daoMock.getByObjectId(bean.getObjectId())).andThrow(
               new DummyRuntimeException("no such id"));
         replay(daoMock);
         manager.get(null,
               Filter.getNewEqualsFilter("objectId", bean.getObjectId()));
   ...
   // @Test (groups = {"withDao"}, description =
   // "manager should remove the bean" )
   // public void verifyRemoveWithDao() throws ChouetteException
   // {
   //
   // manager.setDao(daoMock);
   // manager.remove(null, bean, false);
   // T retBean = manager.get(null, Filter.getNewEqualsFilter("id",
   // bean.getId()), DetailLevelEnum.ATTRIBUTE);
