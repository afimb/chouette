...
public abstract class AbstractImportManagerTests<T extends NeptuneIdentifiedObject>
      extends AbstractManagerTests<T>
{
   ...
   {
      expect(importMock.getDescription()).andReturn(importDescription)
            .anyTimes();
      replay(importMock);
      manager.addImportPlugin(importMock);
      List<FormatDescription> list = manager.getImportFormats(null);
      Assert.assertTrue(list.contains(importDescription),
   ...
   public void verifyDoImport() throws ChouetteException
   {
      ...
      replay(importMock);
      manager.addImportPlugin(importMock);
      List<T> retBeans = manager.doImport(null, importDescription.getName(),
            values, ireport, vreport);
      Assert.assertEquals(retBeans, beans,
            "imported beans should match expected beans");
      verify(importMock);
   }
