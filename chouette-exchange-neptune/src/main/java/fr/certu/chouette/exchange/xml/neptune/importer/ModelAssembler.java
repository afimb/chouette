...
{
   private static final Logger logger = Logger.getLogger(ModelAssembler.class);
    ...
    *           dictionary to fill
    */
   private <T extends NeptuneIdentifiedObject> void populateDictionnary(
         List<T> list, Map<String, T> dictionnary)
   {
   ...
   private void connectRoutingConstraints()
   {
      ...
      {
         if (restriction.getLineId() == null
               || !restriction.getLineId().equals(line.getObjectId()))
         {
            logger.warn("ITL with lineId = " + restriction.getLineId()
                  + ": rejected");
   ...
   private void connectRoutes()
   {
      ...
      {
         route.setJourneyPatterns(getObjectsFromIds(
               route.getJourneyPatternIds(), JourneyPattern.class));
         ...
         if (route.getPtLinks() != null)
         {
            ...
            {
               StopPoint startPoint = getObjectFromId(
                     ptLink.getStartOfLinkId(), StopPoint.class);
               if (startPoint != null)
               {
               ...
               StopPoint endPoint = getObjectFromId(ptLink.getEndOfLinkId(),
                     StopPoint.class);
               if (endPoint != null)
               {
   ...
   private void connectJourneyPatterns()
   {
      ...
      {
         journeyPattern.setStopPoints(getObjectsFromIds(
               journeyPattern.getStopPointIds(), StopPoint.class));
         if (journeyPattern.getStopPoints() == null)
         {
            logger.error("journeyPattern has no stoppoints , rejected "
         ...
         for (StopPoint point : journeyPattern.getStopPoints())
         {
            if (journeyPattern.getArrivalStopPoint() == null
                  || journeyPattern.getArrivalStopPoint().before(point))
            {
               journeyPattern.setArrivalStopPoint(point);
            }
            if (journeyPattern.getDepartureStopPoint() == null
                  || journeyPattern.getDepartureStopPoint().after(point))
            {
         ...
         // Neptune norm said Route must have JourneyPatternId but XSD
         // accepts if missing
   ...
   private void connectPTLinks()
   {
      for (PTLink ptLink : ptLinks)
      {
         ptLink.setStartOfLink(getObjectFromId(ptLink.getStartOfLinkId(),
               StopPoint.class));
         ptLink.setEndOfLink(getObjectFromId(ptLink.getEndOfLinkId(),
               StopPoint.class));
   ...
   private void connectVehicleJourneys()
   {
      ...
      {
         vehicleJourney.setCompany(getObjectFromId(
               vehicleJourney.getCompanyId(), Company.class));
         JourneyPattern journeyPattern = getObjectFromId(
               vehicleJourney.getJourneyPatternId(), JourneyPattern.class);
         ...
         {
            vehicleJourneyAtStop.setStopPoint(getObjectFromId(
                  vehicleJourneyAtStop.getStopPointId(), StopPoint.class));
         ...
         vehicleJourney.setTimeSlot(getObjectFromId(
               vehicleJourney.getTimeSlotId(), TimeSlot.class));
   ...
   private void connectStopPoints()
   {
      ...
      {
         stopPoint.setContainedInStopArea(getObjectFromId(
               stopPoint.getContainedInStopAreaId(), StopArea.class));
         ...
         // if (ptNetwork != null &&
         // ptNetwork.getObjectId().equals(stopPoint.getPtNetworkIdShortcut()))
         // {
         //
         ...
         for (Facility facility : facilities)
         {
            if (facility.getStopPoint() != null
                  && facility.getStopPoint().equals(stopPoint))
   ...
   private void connectStopAreas()
   {
      ...
      {
         AreaCentroid centroid = getObjectFromId(stopArea.getAreaCentroidId(),
               AreaCentroid.class);
         if (centroid != null)
         {
         ...
         {
            if (stopArea.getAreaType().equals(ChouetteAreaEnum.Quay)
                  || stopArea.getAreaType().equals(
                        ChouetteAreaEnum.BoardingPosition))
            {
               stopArea.setContainedStopPoints(getObjectsFromIds(
                     stopArea.getContainedStopIds(), StopPoint.class));
               stopArea.setContainedStopAreas(null);
            } else
            {
               stopArea.setContainedStopAreas(getObjectsFromIds(
                     stopArea.getContainedStopIds(), StopArea.class));
         ...
         {
            stopArea.setRoutingConstraintAreas(getObjectsFromIds(
                  stopArea.getContainedStopIds(), StopArea.class));
            stopArea.setContainedStopAreas(null);
            stopArea.setContainedStopPoints(null);
         }
         ...
         for (Facility facility : facilities)
         {
            if (facility.getStopArea() != null
                  && facility.getStopArea().equals(stopArea))
   ...
   private void connectAreaCentroids()
   {
      ...
      {
         areaCentroid.setContainedInStopArea(getObjectFromId(
               areaCentroid.getContainedInStopAreaId(), StopArea.class));
      }
   }
   ...
   private void connectConnectionLinks()
   {
      ...
      {
         StopArea startOfLink = getObjectFromId(
               connectionLink.getStartOfLinkId(), StopArea.class);
         if (startOfLink != null)
         {
            connectionLink.setStartOfLink(startOfLink);
            startOfLink.addConnectionLink(connectionLink);
         }
         StopArea endOfLink = getObjectFromId(connectionLink.getEndOfLinkId(),
               StopArea.class);
         if (endOfLink != null)
         {
         ...
         for (Facility facility : facilities)
         {
            if (facility.getConnectionLink() != null
                  && facility.getConnectionLink().equals(connectionLink))
   ...
   private void connectTimetables()
   {
      ...
      {
         timetable.setVehicleJourneys(getObjectsFromIds(
               timetable.getVehicleJourneyIds(), VehicleJourney.class));
         if (timetable.getVehicleJourneys() == null
               || timetable.getVehicleJourneys().isEmpty())
         {
            List<DbVehicleJourney> journeys = getObjectsFromIds(
                  timetable.getVehicleJourneyIds(), DbVehicleJourney.class);
            if (journeys != null)
            {
         ...
         {
            logger.warn("timetable " + timetable.getComment() + " ("
                  + timetable.getObjectId() + " ) has no VehicleJourney");
         }
      }
   }
   ...
   private void connectAccessLinks()
   {
      ...
      {
         StopArea stopArea = (getObjectFromId(accessLink.getStartOfLinkId(),
               StopArea.class) != null) ? getObjectFromId(
               accessLink.getStartOfLinkId(), StopArea.class)
         ...
         AccessPoint accessPoint = (getObjectFromId(
               accessLink.getStartOfLinkId(), AccessPoint.class) != null) ? getObjectFromId(
               accessLink.getStartOfLinkId(), AccessPoint.class)
   ...
   private void connectAccessPoints()
   {
      ...
      {
         StopArea stopArea = getObjectFromId(
               accessPoint.getContainedInStopArea(), StopArea.class);
         if (stopArea != null)
         {
   ...
   private void connectGroupOfLines()
   {
      if (groupOfLines != null)
      {
         for (GroupOfLine groupOfLine : groupOfLines)
         {
            groupOfLine.setLines(getObjectsFromIds(groupOfLine.getLineIds(),
                  Line.class));
         }
      }
   ...
   private void connectFacilities()
   {
      for (Facility facility : facilities)
      {
         ...
         {
            StopArea bean = getObjectFromId(facility.getContainedIn(),
                  StopArea.class);
            if (bean != null)
            {
         ...
         {
            StopArea bean = getObjectFromId(facility.getStopAreaId(),
                  StopArea.class);
            if (bean != null)
            {
         ...
         {
            StopPoint bean = getObjectFromId(facility.getStopPointId(),
                  StopPoint.class);
            if (bean != null)
            {
         ...
         {
            ConnectionLink bean = getObjectFromId(
                  facility.getConnectionLinkId(), ConnectionLink.class);
            if (bean != null)
            {
   ...
   private <T extends NeptuneIdentifiedObject> List<T> getObjectsFromIds(
         List<String> ids, Class<T> dictionaryClass)
   {
      Map<String, ? extends NeptuneIdentifiedObject> dictionary = populatedDictionaries
            .get(dictionaryClass);
    ...
    */
   @SuppressWarnings("unchecked")
   private <T extends NeptuneIdentifiedObject> T getObjectFromId(String id,
         Class<T> dictionaryClass)
   {
      ...
      {
         logger.warn("object not found " + id + " in "
   ...
   // private void addValidationError(String checkPointKey,DetailReportItem
   // item)
   // {
   // CheckPointReportItem checkPoint =
   // validationReport.getItem(checkPointKey);
