...
public class Level2Validator
{
   ...
   @Setter
   private PTNetworkType ptNetwork;

   @Setter
   private ChouettePTNetworkType.ChouetteLineDescription.Line line;
   ...
   public void addAccessLinks(
         List<ChouettePTNetworkType.AccessLink> accessLinkList)
   {
   ...
   public void addAreaCentroids(
         List<ChouettePTNetworkType.ChouetteArea.AreaCentroid> areaCentroidList)
   {
   ...
   public void addConnectionLinks(
         List<ChouettePTNetworkType.ConnectionLink> connectionLinkList)
   {
   ...
   public void addPTLinks(List<PTLinkType> ptLinkList)
   {
      for (PTLinkType ptLink : ptLinkList)
      {
         ...
         {
            List<PTLinkType> list = mapPTLinksByStartId.get(ptLink
                  .getStartOfLink());
            if (list == null)
            {
         ...
         {
            List<PTLinkType> list = mapPTLinksByEndId
                  .get(ptLink.getEndOfLink());
            if (list == null)
            {
               list = new ArrayList<PTLinkType>();
   ...
   public void addRoutingConstraints(List<ITLType> routingConstraintList)
   {
      ...
      {
         routingConstraints.put(routingConstraint.getAreaId(),
               routingConstraint);
      }
   }
   ...
   public void addStopAreas(
         List<ChouettePTNetworkType.ChouetteArea.StopArea> stopAreaList)
   {
      for (ChouettePTNetworkType.ChouetteArea.StopArea stopArea : stopAreaList)
      {
         if (stopArea.getStopAreaExtension() == null)
            continue;
         if (stopArea.getStopAreaExtension().getAreaType() == null)
            continue;

         stopAreas.put(stopArea.getObjectId(), stopArea);
         tridentObjects.put(stopArea.getObjectId(), stopArea);
         if (!stopArea.getStopAreaExtension().getAreaType()
               .equals(ChouetteAreaType.ITL))
         {
            if (stopArea.getContains() == null)
               continue;
   ...
   public void addStopPoints(
         List<ChouettePTNetworkType.ChouetteLineDescription.StopPoint> stopPointList)
   {
   ...
   public void validate()
   {
      ...
      // long duration = (endTime - startStepTIme) / 1000;
      // long millis = (endTime - startStepTIme) % 1000;
      // log.info("validating GourpOfLines in "+duration+" s "+millis +
      // " ms");
      // startStepTIme = endTime;
      // }
      validateStopAreas();
      ...
      // long duration = (endTime - startStepTIme) / 1000;
      // long millis = (endTime - startStepTIme) % 1000;
      // log.info("validating AreaCentroids in "+duration+" s "+millis +
      // " ms");
      // startStepTIme = endTime;
      // }
      ...
      // long duration = (endTime - startStepTIme) / 1000;
      // long millis = (endTime - startStepTIme) % 1000;
      // log.info("validating ConnectionLinks in "+duration+" s "+millis +
      // " ms");
      // startStepTIme = endTime;
      // }
      ...
      // long duration = (endTime - startStepTIme) / 1000;
      // long millis = (endTime - startStepTIme) % 1000;
      // log.info("validating AccessPoints in "+duration+" s "+millis +
      // " ms");
      // startStepTIme = endTime;
      // }
      validateAccessLink();
      ...
      // long duration = (endTime - startStepTIme) / 1000;
      // long millis = (endTime - startStepTIme) % 1000;
      // log.info("validating JourneyPatterns in "+duration+" s "+millis +
      // " ms");
      // startStepTIme = endTime;
      // }
      ...
      // long duration = (endTime - startStepTIme) / 1000;
      // long millis = (endTime - startStepTIme) % 1000;
      // log.info("validating VehicleJourneys in "+duration+" s "+millis +
      // " ms");
      // startStepTIme = endTime;
      // }
      validateFacilities();
   ...
   private void validateStopAreas()
   {
      ...
      for (StopArea stopArea : stopAreas.values())
      {
         ...
         for (String childId : stopArea.getContains())
         {
            ...
            if (trdObject == null)
               continue; // external object, cannot check
            if (!(trdObject instanceof StopArea)
                  && !(trdObject instanceof StopPoint))
            {
               ...
               DetailReportItem errorItem = new DetailReportItem(STOP_AREA_1,
            ...
            } else
            {
               ...
               case STOP_PLACE:
                  ...
                  if (trdObject instanceof StopPoint
                        || (!typeOfStopArea(trdObject).equals(
                              ChouetteAreaType.STOP_PLACE) && !typeOfStopArea(
                              trdObject).equals(
                              ChouetteAreaType.COMMERCIAL_STOP_POINT)))
                  {
                     // wrong reference type
                     Locator trdLocation = stopArea.sourceLocation();
                     Map<String, Object> map = new HashMap<String, Object>();
                     ...
                     map.put("parentType",
                           ChouetteAreaType.STOP_PLACE.toString());
                     ReportLocation location = new ReportLocation(sourceFile,
                           trdLocation.getLineNumber(),
                           trdLocation.getColumnNumber());
                     DetailReportItem errorItem = new DetailReportItem(
                           STOP_AREA_2, stopArea.getObjectId(),
                           Report.STATE.ERROR, location, map);
               ...
               case COMMERCIAL_STOP_POINT:
                  ...
                  if (trdObject instanceof StopPoint
                        || (!typeOfStopArea(trdObject).equals(
                              ChouetteAreaType.QUAY) && !typeOfStopArea(
                              trdObject).equals(
                              ChouetteAreaType.BOARDING_POSITION)))
                  {
                     // wrong reference type
                     Locator trdLocation = stopArea.sourceLocation();
                     Map<String, Object> map = new HashMap<String, Object>();
                     ...
                     map.put("type", getType(trdObject));
                     map.put("parentType",
                           ChouetteAreaType.COMMERCIAL_STOP_POINT.toString());
                     ReportLocation location = new ReportLocation(sourceFile,
                           trdLocation.getLineNumber(),
                           trdLocation.getColumnNumber());
                     DetailReportItem errorItem = new DetailReportItem(
                           STOP_AREA_3, stopArea.getObjectId(),
                           Report.STATE.ERROR, location, map);
               ...
               case BOARDING_POSITION:
                  ...
                  if (trdObject instanceof StopArea)
                  {
                     ...
                     ReportLocation location = new ReportLocation(sourceFile,
                           ...
                           trdLocation.getColumnNumber());
                     DetailReportItem errorItem = new DetailReportItem(
                           STOP_AREA_4, stopArea.getObjectId(),
                           Report.STATE.ERROR, location, map);
               ...
               case ITL:
                              ...
                              ChouetteAreaType.ITL))
                  {
                     ...
                     ReportLocation location = new ReportLocation(sourceFile,
                           trdLocation.getLineNumber(),
                           trdLocation.getColumnNumber());
                     DetailReportItem errorItem = new DetailReportItem(ITL_1,
                           stopArea.getObjectId(), Report.STATE.ERROR,
                           location, map);
         ...
         case ITL:
         {
            ...
            if (!found)
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(ITL_2,
                     stopArea.getObjectId(), Report.STATE.ERROR, location, null);
         ...
         case BOARDING_POSITION:
            if (stopArea.isSetCentroidOfArea())
            {
               ...
               // it refers an existing araacentroid (replace test
               // fk_centroid_stoparea from XSD)
               AreaCentroid centroid = areaCentroids.get(stopArea
                     .getCentroidOfArea());
               if (centroid == null)
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        ...
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(
                        STOP_AREA_5, stopArea.getObjectId(),
                        Report.STATE.ERROR, location, map);
               ...
               } else
               {
                  ...
                  if (centroid.isSetContainedIn())
                  {
                     ...
                     if (!centroid.getContainedIn().equals(
                           stopArea.getObjectId()))
                     {
                        ...
                        ReportLocation location = new ReportLocation(
                              sourceFile, trdLocation.getLineNumber(),
                              trdLocation.getColumnNumber());
                        DetailReportItem errorItem = new DetailReportItem(
                              STOP_AREA_6, stopArea.getObjectId(),
                              Report.STATE.ERROR, location, map);
      ...
      if (!routingConstraints.isEmpty())
      {
         ...
         for (ITLType itl : routingConstraints.values())
         {
            ...
            if (area == null)
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(ITL_3,
                     Report.STATE.ERROR, location, map);
            ...
            } else
            {
               ...
               if (!typeOfStopArea(area).equals(ChouetteAreaType.ITL))
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        trdLocation.getLineNumber(),
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(ITL_4,
                        Report.STATE.ERROR, location, map);
            ...
            if (itl.isSetLineIdShortCut())
            {
               ...
               if (!itl.getLineIdShortCut().equals(line.getObjectId()))
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        trdLocation.getLineNumber(),
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(ITL_5,
                        Report.STATE.ERROR, location, map);
   ...
   private void validateAreaCentroids()
   {
      ...
      for (AreaCentroid centroid : areaCentroids.values())
      {
         ...
         if (stopAreas.get(centroid.getContainedIn()) == null)
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(AREA_CENTROID_1,
                  centroid.getObjectId(), Report.STATE.ERROR, location, map);
      ...
      for (AreaCentroid centroid : areaCentroids.values())
      {
         if (centroid.getLongLatType().equals(LongLatTypeType.WGS_84))
            continue;
         Locator trdLocation = centroid.sourceLocation();
         Map<String, Object> map = new HashMap<String, Object>();
         ...
         ReportLocation location = new ReportLocation(sourceFile,
               trdLocation.getLineNumber(), trdLocation.getColumnNumber());
         DetailReportItem errorItem = new DetailReportItem(AREA_CENTROID_2,
               centroid.getObjectId(), Report.STATE.ERROR, location, map);
   ...
   private void validateConnectionLinks()
   {
      ...
      for (ConnectionLink connectionLink : connectionLinks.values())
      {
         ...
         ReportLocation location = new ReportLocation(sourceFile,
               trdLocation.getLineNumber(), trdLocation.getColumnNumber());
         DetailReportItem errorItem = new DetailReportItem(CONNECTION_LINK_1,
               connectionLink.getObjectId(), Report.STATE.ERROR, location, map);
   ...
   private void validateAccessPoints()
   {
      ...
      for (PTAccessPointType accessPoint : accessPoints.values())
      {
         ...
         if (parent == null)
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(ACCESS_POINT_1,
         ...
         } else
         {
            ...
            if (typeOfStopArea(parent).equals(ChouetteAreaType.ITL))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     ACCESS_POINT_2, accessPoint.getObjectId(),
                     Report.STATE.ERROR, location, map);
         ...
         if (links == null)
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(ACCESS_POINT_3,
                  accessPoint.getObjectId(), Report.STATE.ERROR, location, null);
         ...
         } else
         {
            ...
            {
               if (link.getStartOfLink().equals(objectId))
                  startFound = true;
               if (link.getEndOfLink().equals(objectId))
                  endFound = true;
            }
            ...
            if (accessPoint.getType().equalsIgnoreCase("in"))
            {
               ...
               if (endFound)
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        ...
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(
                        ACCESS_POINT_4, accessPoint.getObjectId(),
                        Report.STATE.ERROR, location, null);
            ...
            } else if (accessPoint.getType().equalsIgnoreCase("out"))
            {
               ...
               if (startFound)
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        ...
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(
                        ACCESS_POINT_5, accessPoint.getObjectId(),
                        Report.STATE.ERROR, location, null);
            ...
            // inout
            {
               ...
               if (!startFound || !endFound)
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        ...
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(
                        ACCESS_POINT_6, accessPoint.getObjectId(),
                        Report.STATE.ERROR, location, null);
         ...
         if (accessPoint.getLongLatType().equals(LongLatTypeType.WGS_84))
            continue;
         Locator trdLocation = accessPoint.sourceLocation();
         Map<String, Object> map = new HashMap<String, Object>();
         ...
         ReportLocation location = new ReportLocation(sourceFile,
               trdLocation.getLineNumber(), trdLocation.getColumnNumber());
         DetailReportItem errorItem = new DetailReportItem(ACCESS_POINT_7,
               accessPoint.getObjectId(), Report.STATE.ERROR, location, map);
   ...
   private void validateAccessLink()
   {
      ...
      {
         boolean step1 = true;
         if (!stopAreas.containsKey(link.getStartOfLink())
               && !accessPoints.containsKey(link.getStartOfLink()))
         {
         ...
         if (!stopAreas.containsKey(link.getEndOfLink())
               && !accessPoints.containsKey(link.getEndOfLink()))
         {
            Locator trdLocation = link.sourceLocation();
            Map<String, Object> map = new HashMap<String, Object>();
         ...
         // other as stoparea
         prepareCheckPoint(ACCESS_LINK_2);
         TridentObjectType startObject = tridentObjects.get(link
               .getStartOfLink());
   ...
   {
      boolean route1ok = true; // indicate if ptlinks of routes are correctly
      // defined
      boolean route2ok = true; // indicate if stoppoints are correctly used in
      // ptlinks
      ...
      for (ChouetteRoute route : routes.values())
      {
         for (String ptLinkId : route.getPtLinkId())
         {
            if (!ptLinks.containsKey(ptLinkId))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(ROUTE_2,
                     route.getObjectId(), Report.STATE.ERROR, location, map);
            ...
            if (ptLinkInRoute.get(ptLinkId) != null)
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(ROUTE_4,
                     route.getObjectId(), Report.STATE.ERROR, location, map);
            ...
            for (String journeyPatternId : route.getJourneyPatternId())
            {
               if (!journeyPatterns.containsKey(journeyPatternId))
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        trdLocation.getLineNumber(),
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(ROUTE_1,
                        route.getObjectId(), Report.STATE.ERROR, location, map);
      ...
      for (String stopPointId : mapPTLinksByStartId.keySet())
      {
         ...
         for (PTLinkType ptLinkType : ptLinkOfStop)
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(ROUTE_5,
                  ptLinkType.getObjectId(), Report.STATE.ERROR, location, map);
      ...
      if (!journeyPatterns.isEmpty())
      {
         ...
         for (JourneyPatternType journeyPattern : journeyPatterns.values())
         {
            ...
            if (route != null
                  && !route.getJourneyPatternId().contains(
                        journeyPattern.getObjectId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(ROUTE_7,
                     route.getObjectId(), Report.STATE.ERROR, location, map);
      ...
      if (route1ok && route2ok)
      {
         ...
         for (ChouetteRoute route : routes.values())
         {
            ...
            boolean route3ok = true; // journey pattern stopPoints should be
            // checked
               ...
               if (!mapPTLinksByEndId.containsKey(link.getStartOfLink()))
               {
                  ...
                  } else
                  {
                     ...
                     ReportLocation location = new ReportLocation(sourceFile,
                           trdLocation.getLineNumber(),
                           trdLocation.getColumnNumber());
                     DetailReportItem errorItem = new DetailReportItem(ROUTE_6
                           + "_2", route.getObjectId(), Report.STATE.ERROR,
                           location, map);
            ...
            if (startLink == null)
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     ROUTE_6 + "_1", route.getObjectId(), Report.STATE.ERROR,
                     location, null);
               addValidationError(ROUTE_6, errorItem);
               route3ok = false;
            } else
            {
               ...
               List<PTLinkType> links = mapPTLinksByStartId.get(startLink
                     .getEndOfLink());
         ...
         for (ChouetteRoute route : routes.values())
         {
            if (route.getWayBackRouteId() != null)
            {
               ...
               ChouetteRoute wayBackRoute = routes.get(route
                     .getWayBackRouteId());
               if (wayBackRoute == null)
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        trdLocation.getLineNumber(),
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(ROUTE_3,
                        route.getObjectId(), Report.STATE.ERROR, location, map);
               ...
               // routes
               if (wayBackRoute.getWayBackRouteId() == null
                     || !wayBackRoute.getWayBackRouteId().equals(
                           route.getObjectId()))
               {
                  Locator trdLocation = route.sourceLocation();
                  Map<String, Object> map = new HashMap<String, Object>();
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        trdLocation.getLineNumber(),
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(ROUTE_10,
                        route.getObjectId(), Report.STATE.ERROR, location, map);
               ...
               if (route.getRouteExtension() != null
                     && wayBackRoute.getRouteExtension() != null)
               {
                  prepareCheckPoint(ROUTE_11);
                  String wk1 = route.getRouteExtension().getWayBack()
                        .toLowerCase().substring(0, 1);
                  String wk2 = wayBackRoute.getRouteExtension().getWayBack()
                        .toLowerCase().substring(0, 1);
                  if (wk1.equals(wk2))
                  {
                     Locator trdLocation = route.sourceLocation();
                     Map<String, Object> map = new HashMap<String, Object>();
                     map.put("waybackRouteId", route.getWayBackRouteId());
                     map.put("waybackValue", route.getRouteExtension()
                           .getWayBack());
                     map.put("oppositeWaybackValue", wayBackRoute
                           .getRouteExtension().getWayBack());
                     ReportLocation location = new ReportLocation(sourceFile,
                           trdLocation.getLineNumber(),
                           trdLocation.getColumnNumber());
                     DetailReportItem errorItem = new DetailReportItem(
                           ROUTE_11, route.getObjectId(), Report.STATE.WARNING,
                           location, map);
               ...
               List<String> pointIds = sequenceOfRoutes
                     .get(route.getObjectId());
               List<String> wbPointIds = sequenceOfRoutes.get(wayBackRoute
                     .getObjectId());
               if (pointIds != null && wbPointIds != null)
               {
                  ...
                  StopPoint end = stopPoints.get(wbPointIds.get(wbPointIds
                        .size() - 1));
                  if (end.getContainedIn().equals(start.getContainedIn()))
                     continue;
                  String startParentCommercialId = mapStopAreaParent.get(start
                        .getContainedIn());
                  String endParentCommercialId = mapStopAreaParent.get(end
                        .getContainedIn());
                  if (startParentCommercialId == null
                        || endParentCommercialId == null)
                     continue;
                  if (startParentCommercialId.equals(endParentCommercialId))
                     continue;
                  // warning
                  Locator trdLocation = route.sourceLocation();
                  Map<String, Object> map = new HashMap<String, Object>();
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        trdLocation.getLineNumber(),
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(ROUTE_12,
   ...
   private void validatePtLink()
   {
      ...
      for (PTLinkType link : ptLinks.values())
      {
         if (!stopPoints.containsKey(link.getStartOfLink()))
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(PT_LINK_1,
                  link.getObjectId(), Report.STATE.ERROR, location, map);
         ...
         if (journeyPattern.isSetRouteId())
         {
            if (!routes.containsKey(journeyPattern.getRouteId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     JOURNEY_PATTERN_1, journeyPattern.getObjectId(),
                     Report.STATE.ERROR, location, map);
         ...
         for (String stopPointId : journeyPattern.getStopPointList())
         {
            if (!stopPoints.containsKey(stopPointId))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     JOURNEY_PATTERN_2, journeyPattern.getObjectId(),
                     Report.STATE.ERROR, location, map);
         ...
         if (journeyPattern.isSetLineIdShortcut())
         {
            ...
            if (!line.getObjectId().equals(journeyPattern.getLineIdShortcut()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     JOURNEY_PATTERN_3, journeyPattern.getObjectId(),
                     Report.STATE.ERROR, location, map);
   ...
   private void validateStopPoints()
   {
      ...
      for (StopPoint point : stopPoints.values())
      {
         ...
         {
            // 2-NEPTUNE-StopPoint-1 : check existence of line referred by
            // lineIdShortcut
            ...
            if (!line.getObjectId().equals(point.getLineIdShortcut()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(STOP_POINT_1,
                     point.getObjectId(), Report.STATE.ERROR, location, map);
         ...
         {
            // 2-NEPTUNE-StopPoint-1 : check existence of PTNetwork referred
            // by ptNetworkIdShortcut
            ...
            if (!ptNetwork.getObjectId().equals(point.getPtNetworkIdShortcut()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(STOP_POINT_2,
                     point.getObjectId(), Report.STATE.ERROR, location, map);
         ...
         if (!stopAreas.containsKey(point.getContainedIn()))
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(STOP_POINT_3,
                  point.getObjectId(), Report.STATE.ERROR, location, map);
         ...
         if (!point.getLongLatType().equals(LongLatTypeType.WGS_84))
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(STOP_POINT_4,
                  point.getObjectId(), Report.STATE.ERROR, location, map);
   ...
   private void validateTimetables()
   {
      ...
      List<String> unreferencedVehicleJourneys = new ArrayList<String>(
            vehicleJourneys.keySet());
      for (TimetableType timetable : timetables.values())
      {
         boolean vjFound = false;
         for (String vjId : timetable.getVehicleJourneyId())
         {
            if (vehicleJourneys.containsKey(vjId))
               vjFound = true;
         ...
         if (!vjFound)
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(TIMETABLE_1,
                  timetable.getObjectId(), Report.STATE.WARNING, location, null);
      ...
      if (!unreferencedVehicleJourneys.isEmpty())
      {
         for (String vjId : unreferencedVehicleJourneys)
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(TIMETABLE_2,
                  vj.getObjectId(), Report.STATE.WARNING, location, null);
   ...
   private void validateVehicleJourneys()
   {
      ...
      List<String> unreferencedJourneyPatterns = new ArrayList<String>(
            journeyPatterns.keySet());
      if (!sequenceOfRoutes.isEmpty())
      {
         // 2-NEPTUNE-VehicleJourneyAtStop-3 : check if stoppoints are
         // coherent with route(e)
      ...
      for (VehicleJourneyType vj : vehicleJourneys.values())
      {
         ...
         if (!routes.containsKey(vj.getRouteId()))
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(
                  VEHICLE_JOURNEY_1, vj.getObjectId(), Report.STATE.ERROR,
                  location, map);
         ...
         if (vj.isSetJourneyPatternId())
         {
            ...
            if (!journeyPatterns.containsKey(vj.getJourneyPatternId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     VEHICLE_JOURNEY_2, vj.getObjectId(), Report.STATE.ERROR,
                     location, map);
         ...
         if (vj.isSetLineIdShortcut())
         {
            ...
            if (!line.getObjectId().equals(vj.getLineIdShortcut()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     VEHICLE_JOURNEY_3, vj.getObjectId(), Report.STATE.ERROR,
                     location, map);
         ...
         if (vj.isSetOperatorId())
         {
            ...
            if (!companies.containsKey(vj.getOperatorId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     VEHICLE_JOURNEY_4, vj.getObjectId(), Report.STATE.ERROR,
                     location, map);
         ...
         if (vj.isSetTimeSlotId())
         {
            ...
            if (!timeSlots.containsKey(vj.getTimeSlotId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     VEHICLE_JOURNEY_5, vj.getObjectId(), Report.STATE.ERROR,
                     location, map);
         ...
         for (VehicleJourneyAtStopType vjas : vjass)
         {
            if (!stopPoints.containsKey(vjas.getStopPointId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     VEHICLE_JOURNEY_AT_STOP_1, vj.getObjectId(),
                     Report.STATE.ERROR, location, map);
            ...
            if (vjas.isSetVehicleJourneyId())
            {
               ...
               if (!vj.getObjectId().equals(vjas.getVehicleJourneyId()))
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        ...
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(
                        VEHICLE_JOURNEY_AT_STOP_2, vj.getObjectId(),
                        Report.STATE.ERROR, location, map);
         ...
         if (!journeyPatterns.isEmpty())
         {
            JourneyPatternType journeyPattern = journeyPatterns.get(vj
                  .getJourneyPatternId());
            if (journeyPattern != null)
            {
               ...
               if (!journeyPattern.getRouteId().equals(vj.getRouteId()))
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        ...
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(
                        VEHICLE_JOURNEY_6, vj.getObjectId(),
                        Report.STATE.ERROR, location, map);
               ...
               } else
               {
                        ...
                        || !stops.containsAll(stopsOfVj))
                  {
                     ...
                     ReportLocation location = new ReportLocation(sourceFile,
                           ...
                           trdLocation.getColumnNumber());
                     DetailReportItem errorItem = new DetailReportItem(
                           VEHICLE_JOURNEY_AT_STOP_4, vj.getObjectId(),
                           Report.STATE.ERROR, location, map);
         ...
         if (sequenceOfRoutes.containsKey(vj.getRouteId()))
         {
            List<String> sequence = new ArrayList<String>(
                  sequenceOfRoutes.get(vj.getRouteId()));
            ...
            if (!sequence.equals(stopsOfVj))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(
                     VEHICLE_JOURNEY_AT_STOP_3, vj.getObjectId(),
                     Report.STATE.ERROR, location, map);
      ...
      if (!unreferencedJourneyPatterns.isEmpty())
      {
         ...
         for (String jpId : unreferencedJourneyPatterns)
         {
            ...
            ReportLocation location = new ReportLocation(sourceFile,
                  trdLocation.getLineNumber(), trdLocation.getColumnNumber());
            DetailReportItem errorItem = new DetailReportItem(
                  VEHICLE_JOURNEY_7, jp.getObjectId(), Report.STATE.WARNING,
                  location, null);
      ...
      for (ChouetteFacilityType facility : facilities.values())
      {
         ...
         if (facility.isSetStopAreaId())
         {
            ...
            if (!stopAreas.containsKey(facility.getStopAreaId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(FACILITY_2,
         ...
         if (facility.isSetLineId())
         {
            ...
            if (!line.getObjectId().equals(facility.getLineId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(FACILITY_3,
                     facility.getObjectId(), Report.STATE.ERROR, location, map);
         ...
         if (facility.isSetConnectionLinkId())
         {
            ...
            if (!connectionLinks.containsKey(facility.getConnectionLinkId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(FACILITY_4,
                     facility.getObjectId(), Report.STATE.ERROR, location, map);
         ...
         if (facility.isSetStopPointId())
         {
            ...
            if (!stopPoints.containsKey(facility.getStopPointId()))
            {
               ...
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(FACILITY_5,
                     facility.getObjectId(), Report.STATE.ERROR, location, map);
         ...
         if (facilityLocation != null)
         {
            if (facilityLocation.isSetContainedIn())
            {
               ...
               if (!stopAreas.containsKey(facilityLocation.getContainedIn()))
               {
                  ...
                  ReportLocation location = new ReportLocation(sourceFile,
                        trdLocation.getLineNumber(),
                        trdLocation.getColumnNumber());
                  DetailReportItem errorItem = new DetailReportItem(FACILITY_1,
                        facility.getObjectId(), Report.STATE.ERROR, location,
                        map);
            ...
            if (!facilityLocation.getLongLatType().equals(
                  LongLatTypeType.WGS_84))
            {
               Locator trdLocation = facilityLocation.sourceLocation();
               Map<String, Object> map = new HashMap<String, Object>();
               map.put("longLatType", facilityLocation.getLongLatType()
                     .toString());
               ReportLocation location = new ReportLocation(sourceFile,
                     trdLocation.getLineNumber(), trdLocation.getColumnNumber());
               DetailReportItem errorItem = new DetailReportItem(FACILITY_6,
                     facility.getObjectId(), Report.STATE.ERROR, location, map);
    ...
    * 
    */
   private class VehicleJourneyAtStopSorter implements
         Comparator<VehicleJourneyAtStopType>
   {
