...
public class PTNetworkProducer extends
      AbstractModelProducer<PTNetwork, PTNetworkType>
{

   @Override
   public PTNetwork produce(Context context, PTNetworkType xmlPTNetwork)
   {
      if (xmlPTNetwork == null)
         return null;

      PTNetwork ptNetwork = new PTNetwork();

      // objectId, objectVersion, creatorId, creationTime
      populateFromCastorNeptune(context, ptNetwork, xmlPTNetwork);

      if (ptNetwork.getObjectId().contains(":PTNetwork:"))
      {
         // correct ptnetwork id when old fashioned form
         ptNetwork.setObjectId(ptNetwork.getObjectId().replace(":PTNetwork:",
      ...
      ptNetwork.setDescription(getNonEmptyTrimedString(xmlPTNetwork
            .getDescription()));
      ...
      ptNetwork.setRegistrationNumber(getRegistrationNumber(context,
            xmlPTNetwork.getRegistration()));

      // SourceName optional
      ptNetwork.setSourceName(getNonEmptyTrimedString(xmlPTNetwork
            .getSourceName()));

      // SourceIdentifier optional
      ptNetwork.setSourceIdentifier(getNonEmptyTrimedString(xmlPTNetwork
            .getSourceIdentifier()));
      ...
      if (xmlPTNetwork.getSourceType() != null)
      {
         try
         {
            ptNetwork.setSourceType(PTNetworkSourceTypeEnum
                  .valueOf(xmlPTNetwork.getSourceType().value()));
         } catch (IllegalArgumentException e)
         {
            ReportItem item = new ExchangeReportItem(
                  ExchangeReportItem.KEY.UNKNOWN_ENUM, Report.STATE.ERROR,
                  "SourceType", xmlPTNetwork.getSourceType().value());
      ...
      PTNetwork sharedBean = getOrAddSharedData(context, ptNetwork, xmlPTNetwork);
      if (sharedBean != null)
         ptNetwork = sharedBean;
