...
public class JourneyPatternProducer extends AbstractJaxbNeptuneProducer<JourneyPatternType, JourneyPattern> {
	...
	public JourneyPatternType produce(JourneyPattern journeyPattern) {
		...
		jaxbJourneyPattern.setLineIdShortcut(journeyPattern.getLineIdShortcut()); //FIXME why not a model object ???
		...
		jaxbJourneyPattern.getStopPointList().addAll(NeptuneIdentifiedObject.extractObjectIds(journeyPattern.getStopPoints()));
