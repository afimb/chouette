      ...
      AbstractJaxbNeptuneProducer<ChouetteRoute, Route>
{
   ...
   public ChouetteRoute produce(Route route)
   {
      ...
      jaxbRoute.getJourneyPatternId()
            .addAll(
                  NeptuneIdentifiedObject.extractObjectIds(route
                        .getJourneyPatterns()));

      jaxbRoute.getPtLinkId().addAll(
            NeptuneIdentifiedObject.extractObjectIds(sort(route.getPtLinks())));
   ...
   private class PtLinkSorter implements Comparator<PTLink>
   {
      ...
      {
         return o1.getStartOfLink().getPosition()
               - o2.getStartOfLink().getPosition();
      }
