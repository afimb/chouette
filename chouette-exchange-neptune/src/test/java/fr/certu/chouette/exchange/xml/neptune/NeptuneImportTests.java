...
public class NeptuneImportTests extends AbstractTestNGSpringContextTests
{
   ...
   @Test(groups = { "ImportLine", "ImportUtf8Line", "ImportUtf8BomLine",
         "ImportBadEncLine", "ImportRCLine", "ImportZipLines",
         "CheckParameters" }, description = "Get a bean from context")
   public void getBean()
   {
   ...
   public void verifyCheckInputFileExists() throws ChouetteException
   {
      ...
      for (ReportItem reportItem : items)
      {
         ...
         if (reportItem.hasItems())
         {
            ...
            {
               if (detail.getMessageKey().equals("FILE_ERROR"))
                  found = true;
            }
         }
      }
   ...
   public void verifyCheckGoodEncoding(String neptuneFileUtf8)
   ...
   public void verifyCheckGoodEncodingWithBom(String neptuneFileUtf8Bom)
         throws ChouetteException
   {
      List<ParameterValue> parameters = new ArrayList<ParameterValue>();
      SimpleParameterValue simpleParameterValue = new SimpleParameterValue(
   ...
   public void verifyCheckBadEncoding(String neptuneFileBadEnc)
         throws ChouetteException
   {
      List<ParameterValue> parameters = new ArrayList<ParameterValue>();
      SimpleParameterValue simpleParameterValue = new SimpleParameterValue(
      ...
      printReport(vreport.getReport());
      Assert.assertNull(lines, "lines must be null");
      Assert.assertEquals(ireport.getReport().getStatus(), Report.STATE.ERROR,
            "report status must be ERROR");
   }
      ...
      for (Line line : lines)
      {
         ...
         for (ConnectionLink connectionLink : clinks)
         {
            ...
            Assert.assertEquals(seconds, 4200,
                  "line must have links duration of 1 hour and 10 minutes");
         ...
         {
            Assert.assertNotNull(facility.getFacilityFeatures(),
                  "Facility must have features : " + facility.getObjectId());
            Assert.assertEquals(facility.getFacilityFeatures().size(), 1,
                  "Facility must have 1 feature : " + facility.getObjectId());
            for (FacilityFeature feature : facility.getFacilityFeatures())
            {
               Assert.assertNotNull(feature.getChoiceValue(),
                     "feature must have choice");
               Assert.assertEquals(feature.getAccessFacility(),
                     AccessFacilityEnumeration.BARRIER,
                     "feature must be BARRIER");
            }
         }
   ...
   public void verifyImportRCLine(String neptuneRCFile)
         throws ChouetteException
   {
      ...
      parameters.add(simpleParameterValue);
      SimpleParameterValue simpleParameterValue2 = new SimpleParameterValue(
            "validate");
      simpleParameterValue2.setBooleanValue(false); // file is incomplete in
      // other aspect
      ...
      for (Line line : lines)
      {
         ...
         Assert.assertEquals(area.getAreaType(), ChouetteAreaEnum.ITL,
               "routing constraint area must be of " + ChouetteAreaEnum.ITL
                     + " type");
         ...
         Assert.assertNull(area.getContainedStopAreas(),
               "routing constraint area must not have stopArea children");
         Assert.assertNull(area.getParent(),
               "routing constraint area must not have stopArea parent");
   ...
   @Parameters({ "neptuneZip" })
   @Test(groups = { "ImportZipLines" }, description = "Import Plugin should import zip file", dependsOnMethods = { "getBean" })
   public void verifyImportZipLinesOptim(String neptuneZip)
         throws ChouetteException
   {
   ...
   // @Test (groups = {"ImportZipLines"}, description =
   // "Import Plugin should import zip file") //,dependsOnMethods={"getBean"})
   // public void verifyImportZipLinesErr(String neptuneZipErr) throws
   // ChouetteException
   // {
   // importLine = (IImportPlugin<Line>)
   // applicationContext.getBean("NeptuneLineImport") ;
   //
   // List<ParameterValue> parameters = new ArrayList<ParameterValue>();
   // SimpleParameterValue simpleParameterValue = new
   // SimpleParameterValue("inputFile");
