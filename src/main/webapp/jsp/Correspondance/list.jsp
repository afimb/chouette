...
<s:url id="urlCorrespondances" action="liste_Correspondance" includeParams="none"/>
...
<div class="actions">
	<s:url action="crud_Correspondance!edit" id="createCorrespondance"/>
...
<div class="panel" id="displaytag"> 
	<display:table name="correspondances" pagesize="20"  requestURI="" id="correspondance" export="false">
 		<display:column title="Action" sortable="false">
			<s:url id="removeUrl" action="crud_Correspondance!delete">
				<s:param name="idCorrespondance" value="${correspondance.id}" />
			</s:url>
			<s:url id="editUrl" action="crud_Correspondance!edit">
				<s:param name="idCorrespondance" value="${correspondance.id}" />
			</s:url>
			<s:a href="%{editUrl}"><img border="0" src="images/editer.png" title="<s:text name="tooltip.edit"/>"></s:a>&nbsp;&nbsp;
	   		<s:a href="%{removeUrl}" onclick="return confirm('%{getText('connectionlink.delete.confirmation')}');"><img border="0" src="images/supprimer.png" title="<s:text name="tooltip.delete"/>"></s:a> 	
 		</display:column>	  	
	  	<display:column title="Nom" property="name"/>
	  	<display:column title="Commentaire" property="comment"/>
	  	<display:column title="Type">
	  		<s:text name="${correspondance.linkType}"/>
