...
public class LecteurHoraire implements ILecteurHoraire {
	...
	public void lire(String[] ligneCSV) {
		...
		if (listHorairesParRegistrationLigne.get(ligneCSV[2].trim().substring(0,ligneCSV[2].trim().indexOf('-'))) == null)
		...
		if (!isContigu)
			listHorairesParRegistrationLigne.get(ligneCSV[2].trim().substring(0,ligneCSV[2].trim().indexOf('-'))).add(horaire);
