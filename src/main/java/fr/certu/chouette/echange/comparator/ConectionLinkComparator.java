...
public class ConectionLinkComparator extends AbstractChouetteDataComparator
{
        ...
        for (Correspondance sourceData : sourceDataList) 
        {
            ...
            else
            {
            	if (sourceDataMap.put(key,sourceData) != null)
            	{
            		...
            		//p.println(ComparatorException.TYPE.DuplicateKey + "StartStopName+EndStopName in source ConnectionLink");
            		//p.close();
            		continue;
    ...
    {    	    	    	
    	...
    	{
    		//p.println(data.getObjectId() + "::null-distance");
    	}
    	...
    	{
    		//p.println(data.getObjectId() + "::null-link-type");
    	}
        ...
        if (start == null) 
	    {
        	if (end == null)
        	{
        		//p.println(data.getObjectId() + "::null-key");
        		return null;
        	}
        	//p.println(data.getObjectId() + "::key " + " => " + key);
