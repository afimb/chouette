...
public class AmivifLineComparator extends AbstractAmivifDataComparator
{
            ...
            throws ServiceException
    {
        ...
        if (!state.addAttributeState("LineEnd", Arrays.toString(sourceEnds), Arrays.toString(targetEnds)))
        {
           ...
           state.addAttributeState("unordered LineEnd (order differs)", Arrays.toString(sourceEnds), Arrays.toString(targetEnds));
        }
        ...
        if (!state.addAttributeState("RouteId", Arrays.toString(sourceRoutes),Arrays.toString(targetRoutes)))
        {
           ...
           state.addAttributeState("unordered RouteId (order differs)", Arrays.toString(sourceRoutes), Arrays.toString(targetRoutes));
        }

        // GroupOfLine count
        int sourceGOL = source.getGroupOfLineCount();
        int targetGOL = target.getGroupOfLineCount();        
        state.addAttributeState("GroupOfLine count", sourceGOL, targetGOL);        
        
        // StopPoints in connection count
        String sourceSPIC = ((Integer) source.getStopPointInConnectionCount()).toString();
        String targetSPIC = ((Integer) target.getStopPointInConnectionCount()).toString();
        
        state.addAttributeState("StopPointsInConnection count", sourceSPIC, targetSPIC);        
